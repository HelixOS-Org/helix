...
use crate::{AssetResult, ImportedVertex, ImportedMesh, MeshBounds, Submesh};
...
impl MeshProcessor {
    ...
    pub fn generate_tangents(&self, vertices: &mut [ImportedVertex], indices: &[u32]) {
        ...
        for tri in indices.chunks(3) {
            ...
            let w = if dot(cross(n, tangent), bitangent) < 0.0 { -1.0 } else { 1.0 };
    ...
    /// Generate LOD levels
    pub fn generate_lods(&self, vertices: &[ImportedVertex], indices: &[u32], lod_count: u32) -> Vec<LodLevel> {
    ...
    fn simplify(&self, indices: &[u32], vertices: &[ImportedVertex], target_triangles: usize) -> Vec<u32> {
...
impl MeshletBuilder {
    ...
    fn can_add_triangle(&self, v0: u32, v1: u32, v2: u32) -> bool {
        ...
        if !self.vertex_map.contains_key(&v0) { new_vertices += 1; }
        if !self.vertex_map.contains_key(&v1) { new_vertices += 1; }
        if !self.vertex_map.contains_key(&v2) { new_vertices += 1; }
    ...
    fn build(self, vertices: &[ImportedVertex]) -> Meshlet {
        ...
        let primitive_indices: Vec<u8> = self.triangles
            .iter()
        ...
        let radius = self.vertex_map.keys()
            .filter_map(|&global| {
                vertices.get(global as usize)
                    .map(|v| distance(v.position, center))
            })
            .fold(0.0f32, |a, b| a.max(b));
