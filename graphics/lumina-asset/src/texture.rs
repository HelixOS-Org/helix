...
use crate::{AssetError, AssetErrorKind, AssetResult, TextureFormat, MipmapFilter, TextureCompression};
...
impl TextureProcessor {
    ...
    pub fn generate_mipmaps(&self, width: u32, height: u32, data: &[u8], format: TextureFormat) -> Vec<MipLevel> {
    ...
    pub fn compress_bc7(&self, width: u32, height: u32, data: &[u8]) -> AssetResult<Vec<u8>> {
        compress_block_format(width, height, data, BlockFormat::Bc7, self.config.compression_quality)
    }
    
    /// Compress texture to ASTC
    pub fn compress_astc(&self, width: u32, height: u32, data: &[u8], block_size: AstcBlockSize) -> AssetResult<Vec<u8>> {
        compress_astc_internal(width, height, data, block_size, self.config.compression_quality)
    }
    ...
    /// Resize texture
    pub fn resize(&self, data: &[u8], src_width: u32, src_height: u32, dst_width: u32, dst_height: u32, channels: u32) -> Vec<u8> {
...
fn downsample(
    ...
    match filter {
        ...
        MipmapFilter::Kaiser | MipmapFilter::Lanczos | MipmapFilter::Mitchell | MipmapFilter::Triangle => {
...
impl VirtualTextureSystem {
    ...
    fn evict_lru_page(&mut self) {
        if let Some((&id, _)) = self.pages.iter()
...
impl TextureAtlas {
    pub fn new(width: u32, height: u32, format: TextureFormat) -> Self {
        ...
        Self {
            ...
            free_rects: vec![AtlasRect { x: 0, y: 0, width, height }],
            next_id: 1,
        }
    }
