...
use crate::{AssetId, AssetResult, AssetError, AssetErrorKind, LoadPriority};
...
impl StreamingManager {
    ...
    pub fn get_completed(&mut self) -> Vec<(AssetId, Vec<u8>)> {
        ...
        let complete_ids: Vec<_> = self.active_streams.iter()
        ...
        for id in complete_ids {
            if let Some(stream) = self.active_streams.remove(&id) {
                let data: Vec<u8> = stream.chunks.into_iter()
                    .flat_map(|c| c.data)
                    .collect();
...
impl MipmapStreamer {
    ...
    pub fn request_mip(&mut self, asset_id: AssetId, mip_level: u8) {
        let entry = self.loaded_mips.entry(asset_id)
    ...
    pub fn evict_if_needed(&mut self) -> Vec<(AssetId, u8)> {
        ...
        while self.memory_used > self.memory_budget {
            ...
            if let Some((&id, mips)) = self.loaded_mips.iter()
...
impl LodStreamer {
    ...
    pub fn request_lod(&mut self, asset_id: AssetId, lod_level: u8) {
        let entry = self.loaded_lods.entry(asset_id)
