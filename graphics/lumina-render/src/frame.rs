...
mod tests {
    ...
    fn test_frame_creation() {
        let frame = Frame::new(0, &FrameConfig::default());
        assert_eq!(frame.index(), 0);
        assert_eq!(frame.number(), 0);
    }

    #[test]
    fn test_scratch_allocator() {
        let mut alloc = ScratchAllocator::new(1024);

        let a1 = alloc.allocate(100, 16).unwrap();
        assert_eq!(a1.offset, 0);

        let a2 = alloc.allocate(200, 64).unwrap();
        assert_eq!(a2.offset, 128); // Aligned to 64
    }

    #[test]
    fn test_command_list() {
        let mut cmd = CommandList::new(CommandListType::Graphics);
        cmd.begin();
        cmd.draw(36, 1, 0, 0);
        cmd.dispatch(8, 8, 1);
        cmd.end();
    ...
    #[test]
    fn test_frame_graph_builder() {
        let mut builder = FrameGraphBuilder::new();

        let depth = builder.create_resource("depth", ResourceDesc::Imported);
        let color = builder.create_resource("color", ResourceDesc::Imported);

        let pass = builder.add_pass("main", PassType::Graphics);
        builder.read(pass, depth);
        builder.write(pass, color);
