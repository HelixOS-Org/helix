...
//!
//! This module provides the system call interface for AArch64.
//! System calls use the SVC instruction with the call number in x8.
...
/// Convert syscall result to return value
pub fn syscall_result_to_ret(result: SyscallResult) -> i64 {
    match result {
...
pub type SyscallHandler = fn(usize, usize, usize, usize, usize, usize) -> SyscallResult;
...
impl SyscallTable {
    ...
    /// Get syscall entry
    pub fn get(&self, nr: usize) -> Option<&SyscallEntry> {
        if nr < MAX_SYSCALL {
            self.entries[nr].as_ref()
        } else {
            None
        }
    }
    ...
    /// Invoke a syscall
    pub fn invoke(&self, nr: usize, args: &[usize; 6]) -> SyscallResult {
        match self.get(nr) {
            Some(entry) => (entry.handler)(args[0], args[1], args[2], args[3], args[4], args[5]),
            None => Err(SyscallError::Enosys),
        }
    }
}
...
/// Dispatch system call from trap frame
///
/// Called from exception handler when SVC is detected.
pub fn dispatch_syscall(frame: &mut TrapFrame) -> i64 {
    ...
    // Get syscall table - this would come from a global or per-CPU structure
    ...
    let ret = syscall_result_to_ret(result);
    frame.x0 = ret as u64;
    ret
}

// =============================================================================
// Common Syscall Numbers (Linux ABI compatible)
// =============================================================================

/// Common syscall numbers (Linux ARM64 ABI)
pub mod nr {
...
/// Check if address is valid user pointer
pub fn validate_user_ptr<T>(ptr: *const T) -> Result<(), SyscallError> {
    ...
    // Check if in user space (below kernel address)
    // Kernel addresses start at 0xFFFF_0000_0000_0000
...
/// Check if mutable address is valid
pub fn validate_user_ptr_mut<T>(ptr: *mut T) -> Result<(), SyscallError> {
...
///
/// # Safety
/// Pointer must be valid and readable
pub unsafe fn copy_from_user<T: Copy>(ptr: *const T) -> Result<T, SyscallError> {
...
///
/// # Safety
/// Pointer must be valid and writable
pub unsafe fn copy_to_user<T: Copy>(ptr: *mut T, value: T) -> Result<(), SyscallError> {
    validate_user_ptr_mut(ptr)?;
