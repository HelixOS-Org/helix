...
pub enum ExceptionVector {
    ...
    /// Reserved exception vector (0x16)
    Reserved16           = 0x16,
    /// Reserved exception vector (0x17)
    Reserved17           = 0x17,
    /// Reserved exception vector (0x18)
    Reserved18           = 0x18,
    /// Reserved exception vector (0x19)
    Reserved19           = 0x19,
    /// Reserved exception vector (0x1A)
    Reserved1A           = 0x1A,
    /// Reserved exception vector (0x1B)
    Reserved1B           = 0x1B,
...
impl ExceptionVector {
    ...
    /// Converts a raw vector number to an `ExceptionVector`.
    ///
    /// Returns `None` if the vector is outside the exception range (0x00-0x1F).
    pub const fn from_vector(vector: u8) -> Option<Self> {
        if vector <= 0x1F {
            // Safety: all values 0x00-0x1F are valid enum variants
            Some(unsafe { core::mem::transmute::<u8, ExceptionVector>(vector) })
        } else {
            None
        }
    }

    /// Returns the recommended Interrupt Stack Table (IST) index for this exception.
    ///
    /// Returns 0 if no dedicated IST is recommended (use current stack).
...
impl IrqVector {
    ...
    /// Converts a legacy IRQ number (0-15) to an `IrqVector`.
    ///
    /// Returns `None` if the IRQ number is out of range.
    #[inline]
    pub const fn from_irq(irq: u8) -> Option<Self> {
        if irq < 16 {
            Some(unsafe { core::mem::transmute::<u8, IrqVector>(0x20 + irq) })
        } else {
            None
        }
    }

    /// Returns a human-readable name for this IRQ.
    pub const fn name(self) -> &'static str {
        match self {
...
///
/// These are commonly used APIC vector assignments for device interrupts.
pub mod device {
    /// APIC Timer interrupt vector (0x40).
    pub const APIC_TIMER: u8 = 0x40;

    /// Thermal Sensor interrupt vector (0x41).
    pub const THERMAL: u8 = 0x41;

    /// Performance Monitoring Counter overflow vector (0x42).
    pub const PERF_COUNTER: u8 = 0x42;

    /// Corrected Machine Check Interrupt vector (0x43).
    pub const CMCI: u8 = 0x43;

    /// First available MSI (Message Signaled Interrupt) vector.
    pub const MSI_BASE: u8 = 0x50;

    /// Last available MSI vector.
    pub const MSI_END: u8 = 0xDF;

    /// Total number of available MSI vectors.
    pub const MSI_COUNT: u8 = MSI_END - MSI_BASE + 1;
}

/// Reserved Vectors
///
/// Special-purpose vectors at the top of the vector space.
pub mod reserved {
    /// APIC Error interrupt vector (0xFE).
    ///
    /// Generated by the Local APIC when it detects an error condition.
    pub const APIC_ERROR: u8 = 0xFE;

    /// Spurious Interrupt vector (0xFF).
    ///
    /// Must be set in the APIC spurious interrupt vector register.
...
pub enum Vector {
    /// CPU exception vector in the range 0x00-0x1F.
    Exception(ExceptionVector),

    /// Legacy PIC IRQ vector in the range 0x20-0x2F.
    Irq(IrqVector),

    /// System vector for IPIs, syscalls, and other system purposes.
    System(SystemVector),

    /// Device interrupt vector for APIC, MSI, and MSI-X interrupts.
    Device(u8),
...
impl Vector {
    /// Returns the raw 8-bit vector number.
    #[inline]
    ...
    /// Creates a `Vector` from a raw 8-bit vector number.
    ///
    /// Automatically categorizes the vector based on its value.
