...
pub struct InodeRaw {
    ...
    /* 0xB0 */
    /// Inline data/extents/symlink (64 bytes)
...
impl InodeRaw {
    ...
    pub fn new_device(ino: u64, mode: u32, rdev: u32, uid: u32, gid: u32, now: u64) -> Self {
        let mut inode = Self::new();
        inode.ino = ino;
    ...
    pub fn inline_extents(&self) -> Option<&[Extent]> {
        if self.has_inline_extents() && self.inline_extent_count > 0 {
            ...
            let extents = unsafe {
                core::slice::from_raw_parts(self.inline.as_ptr() as *const Extent, count)
            };
            Some(extents)
        } else {
            None
    ...
    pub fn set_inline_extents(&mut self, extents: &[Extent]) -> HfsResult<()> {
        ...
        let extent_bytes = unsafe {
            core::slice::from_raw_parts(extents.as_ptr() as *const u8, extents.len() * Extent::SIZE)
        };
