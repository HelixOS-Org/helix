...
impl ModuleDesc {
    pub fn new(id: u64, name: String, version: String) -> Self {
        Self {
            ...
            dependents: Vec::new(),
            params: Vec::new(),
            exported_syms: Vec::new(),
            taints: Vec::new(),
            loaded_at: 0,
            srcversion: String::new(),
        }
    }
    ...
    pub fn can_unload(&self) -> bool {
        self.refcount == 0 && self.dependents.is_empty() && self.state == ModuleState::Live
    }
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn add_taint(&mut self, t: ModuleTaint) {
        if !self.taints.contains(&t) {
            self.taints.push(t);
        }
    }
}
...
impl BridgeModuleBridge {
    pub fn new() -> Self {
        Self {
            ...
            events: Vec::new(),
            stats: ModuleBridgeStats::default(),
            next_id: 1,
        }
    }
    ...
    pub fn load_module(&mut self, name: String, version: String, size: u64, ts: u64) -> u64 {
        ...
        self.events.push(ModuleEvent {
            module_id: id,
            kind: ModuleEventKind::Load,
            ts,
            result: 0,
        });
        id
    ...
    pub fn export_symbol(&mut self, mod_id: u64, name: String, addr: u64, gpl: bool, crc: u32) {
        ...
        if let Some(m) = self.modules.get_mut(&mod_id) {
            m.exported_syms.push(name.clone());
        }
    ...
    pub fn add_dependency(&mut self, mod_id: u64, dep_id: u64) {
        ...
        if let Some(d) = self.modules.get_mut(&dep_id) {
            d.dependents.push(mod_id);
            d.refcount += 1;
        }
    }
    ...
    pub fn set_param(&mut self, mod_id: u64, param: &str, val: String, ts: u64) {
        ...
        self.events.push(ModuleEvent {
            module_id: mod_id,
            kind: ModuleEventKind::ParamChange,
            ts,
            result: 0,
        });
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.total_loads = self
            .events
            .iter()
            .filter(|e| e.kind == ModuleEventKind::Load && e.result == 0)
            .count() as u64;
        self.stats.total_unloads = self
            .events
            .iter()
            .filter(|e| e.kind == ModuleEventKind::Unload && e.result == 0)
            .count() as u64;
        self.stats.failed_loads = self
            .events
            .iter()
            .filter(|e| e.kind == ModuleEventKind::Load && e.result < 0)
            .count() as u64;
    ...
    #[inline(always)]
    pub fn module(&self, id: u64) -> Option<&ModuleDesc> {
        self.modules.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &ModuleBridgeStats {
        &self.stats
    }
}
