...
impl ProcessResources {
    ...
    pub fn compare(
        ...
        match rtype {
            ...
            KcmpType::Sighand => {
                if self.sighand_id == other.sighand_id {
                    KcmpResult::Equal
                } else {
                    KcmpResult::NotEqual
                }
...
impl BridgeKcmp {
    ...
    pub fn register_process(&mut self, pid: u64) {
        self.processes
            .entry(pid)
            .or_insert_with(|| ProcessResources::new(pid));
    }
    ...
    pub fn stats(&self) -> KcmpBridgeStats {
        let equal = self
            .history
            .iter()
            .filter(|r| r.result == KcmpResult::Equal)
            .count() as u64;
        let avg_dur = if self.history.is_empty() {
            0
        } else {
        ...
        KcmpBridgeStats {
            ...
            equal_results: equal,
            processes_tracked: self.processes.len() as u32,
            shared_resources: 0,
...
impl BridgeKcmpV2 {
    ...
    pub fn compare(
        ...
        let result = if h1 == h2 {
            ...
            KcmpV2Result::Greater
        };
        if result == KcmpV2Result::Equal {
            self.equal_count += 1;
        }
        ...
        if self.records.len() >= self.max_records {
            self.records.drain(..self.max_records / 2);
        }
        self.records.push(KcmpV2Record {
            id,
            pid1,
            pid2,
            cmp_type,
    ...
    pub fn stats(&self) -> KcmpV2BridgeStats {
        KcmpV2BridgeStats {
            comparisons: self.records.len() as u64,
            equal_results: self.equal_count,
