...
impl ClockSource {
    pub fn new(id: u64, name: String, freq: u64, rating: ClockRating) -> Self {
        let shift = 20u32;
        let mult = if freq > 0 {
            ((1u64 << shift) * 1_000_000_000 / freq) as u32
        } else {
            1
        };
        Self {
    ...
    pub fn is_stable(&self) -> bool {
        !self.flags.contains(&ClockFlag::Unstable)
    }
    #[inline(always)]
    pub fn is_hres_valid(&self) -> bool {
        self.flags.contains(&ClockFlag::ValidForHres)
    }
    #[inline(always)]
...
impl ClockEventDevice {
    pub fn new(id: u64, name: String, cpu: u32, freq: u64) -> Self {
        Self {
            id,
            name,
            rating: ClockRating::Normal,
            mode: ClockEventMode::Unused,
            cpu_id: cpu,
            freq_hz: freq,
            min_delta_ns: 1000,
            max_delta_ns: 1_000_000_000_000,
            event_count: 0,
            next_event_ns: 0,
        }
    }
    ...
    #[inline(always)]
    pub fn program_event(&mut self, ns: u64) {
...
impl BridgeClockBridge {
    pub fn new() -> Self {
        Self {
            ...
            events: BTreeMap::new(),
            timekeeping: TimekeepingState::new(),
            current_source: 0,
            watchdog_source: None,
            stats: ClockBridgeStats::default(),
            next_id: 1,
        }
    }
    ...
    pub fn register_event_device(&mut self, name: String, cpu: u32, freq: u64) -> u64 {
        ...
        self.next_id += 1;
        self.events
            .insert(id, ClockEventDevice::new(id, name, cpu, freq));
        id
    }
    ...
    fn select_best_source(&mut self) {
        ...
        if let Some(b) = best {
            self.current_source = b.id;
            self.timekeeping.clock_id = b.id;
        }
    }

    #[inline(always)]
    pub fn read_clock(&mut self, cycles: u64) -> u64 {
        if let Some(s) = self.sources.get_mut(&self.current_source) {
            s.read(cycles)
        } else {
            0
        }
    ...
    pub fn mark_unstable(&mut self, id: u64) {
        if let Some(s) = self.sources.get_mut(&id) {
            if !s.flags.contains(&ClockFlag::Unstable) {
                s.flags.push(ClockFlag::Unstable);
            }
        }
        if self.current_source == id {
            self.select_best_source();
        }
    }

    #[inline(always)]
    pub fn set_watchdog(&mut self, id: u64) {
        self.watchdog_source = Some(id);
    }

    #[inline(always)]
    pub fn program_timer(&mut self, dev_id: u64, ns: u64) {
        if let Some(d) = self.events.get_mut(&dev_id) {
            d.program_event(ns);
        }
    }
    ...
    #[inline(always)]
    pub fn update_wall_time(&mut self, sec: u64, nsec: u64) {
        self.timekeeping.update_wall(sec, nsec);
    }
    ...
    #[inline(always)]
    pub fn source(&self, id: u64) -> Option<&ClockSource> {
        self.sources.get(&id)
    }
    #[inline(always)]
    pub fn event_device(&self, id: u64) -> Option<&ClockEventDevice> {
        self.events.get(&id)
    }
    #[inline(always)]
    pub fn timekeeping(&self) -> &TimekeepingState {
        &self.timekeeping
    }
    #[inline(always)]
    pub fn stats(&self) -> &ClockBridgeStats {
        &self.stats
    }
}
