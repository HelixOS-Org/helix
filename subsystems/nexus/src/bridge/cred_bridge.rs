...
impl ProcessCred {
    ...
    pub fn is_privileged(&self) -> bool {
        self.euid == 0 || self.caps.effective != 0
    }
    #[inline(always)]
    pub fn in_group(&self, gid: u32) -> bool {
        self.egid == gid || self.supplementary_groups.contains(&gid)
    }
}
...
impl BridgeCred {
    ...
    #[inline(always)]
    pub fn register(&mut self, cred: ProcessCred) {
        self.creds.insert(cred.pid, cred);
    }
    #[inline(always)]
    ...
    pub fn stats(&self) -> CredBridgeStats {
        ...
        CredBridgeStats {
            tracked_processes: self.creds.len() as u32,
            privileged_processes: priv_count,
            cred_changes: self.events.len() as u64,
            setuid_calls: self
                .events
                .iter()
                .filter(|e| e.cred_type == CredType::Real)
                .count() as u64,
        }
    }
}
...
impl BridgeCredV2 {
    ...
    #[inline(always)]
    pub fn track(&mut self, pid: u64, uid: u32, gid: u32) {
        self.creds.insert(pid, ProcessCredV2::new(pid, uid, gid));
    }
    #[inline(always)]
    pub fn setuid(&mut self, pid: u64, uid: u32) {
        if let Some(c) = self.creds.get_mut(&pid) {
            c.euid = uid;
            c.fsuid = uid;
        }
    ...
    #[inline(always)]
    pub fn setgid(&mut self, pid: u64, gid: u32) {
        if let Some(c) = self.creds.get_mut(&pid) {
            c.egid = gid;
            c.fsgid = gid;
        }
