...
impl BridgeCredMgr {
    ...
    pub fn check_access(
        ...
        if let Some(proc_creds) = self.processes.get(&pid) {
            if proc_creds.creds.is_root() {
                return true;
            }
            if let Some(uid) = required_uid {
                if proc_creds.creds.effective_uid != uid {
                    return false;
                }
            }
            if let Some(gid) = required_gid {
                if !proc_creds.creds.in_group(gid) {
                    return false;
                }
            }
            true
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.root_processes = self
            .processes
            .values()
            .filter(|p| p.creds.is_root())
            .count();
    ...
    #[inline(always)]
    pub fn process_creds(&self, pid: u64) -> Option<&ProcessCreds> {
        self.processes.get(&pid)
    }
    #[inline(always)]
    pub fn stats(&self) -> &BridgeCredMgrStats {
        &self.stats
    }
}
