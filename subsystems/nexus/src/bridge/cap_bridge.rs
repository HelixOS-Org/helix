...
impl ProcessCaps {
    pub fn new(pid: u64) -> Self {
        Self {
            pid,
            effective: CapBitmask::empty(),
            permitted: CapBitmask::empty(),
            inheritable: CapBitmask::empty(),
            bounding: CapBitmask::full(),
            ambient: CapBitmask::empty(),
            securebits: 0,
            no_new_privs: false,
        }
    }
...
impl BridgeCap {
    ...
    pub fn register(&mut self, pid: u64) {
        self.processes.insert(pid, ProcessCaps::new(pid));
    }
    ...
    pub fn stats(&self) -> CapBridgeStats {
        let privileged = self
            .processes
            .values()
            .filter(|p| p.effective.count() > 0)
            .count() as u32;
        let caps: Vec<f64> = self
            .processes
            .values()
            .map(|p| p.effective.count() as f64)
            .collect();
        let avg = if caps.is_empty() {
            0.0
        } else {
...
impl CapSetV2 {
    ...
    #[inline(always)]
    pub fn has_effective(&self, bit: u32) -> bool {
        self.effective & (1u64 << bit) != 0
    }
    #[inline(always)]
    pub fn grant(&mut self, bit: u32) {
        self.effective |= 1u64 << bit;
        self.permitted |= 1u64 << bit;
    }
    #[inline(always)]
...
impl BridgeCapV2 {
    ...
    pub fn register(&mut self, pid: u64, caps: CapSetV2) {
        self.processes.insert(pid, ProcessCapsV2 {
            pid,
            caps,
            no_new_privs: false,
            securebits: 0,
    ...
    pub fn check(&self, pid: u64, bit: u32) -> bool {
        self.processes
            .get(&pid)
            .map_or(false, |p| p.caps.has_effective(bit))
    }
