...
#[derive(Debug, Clone)]
#[repr(align(64))]
pub struct BridgeMunmapStats {
    pub total_ops: u64,
    pub successful: u64,
    pub partial: u64,
    pub failed: u64,
    pub total_unmapped_bytes: u64,
}
...
impl BridgeMunmapManager {
    ...
    #[inline(always)]
    pub fn track_map(&mut self, addr: u64, length: u64) {
        self.mapped_regions.insert(addr, length);
    }
    ...
    pub fn munmap(&mut self, addr: u64, length: u64) -> BridgeMunmapResult {
        ...
        if let Some(mapped_len) = self.mapped_regions.get(addr) {
            if length >= mapped_len {
                self.mapped_regions.remove(addr);
            ...
            self.stats.failed += 1;
            BridgeMunmapResult::NotMapped
        }
    }

    #[inline(always)]
    pub fn stats(&self) -> &BridgeMunmapStats {
        &self.stats
    }
}
