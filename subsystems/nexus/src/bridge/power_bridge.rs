...
impl PowerDomain {
    pub fn new(id: u64, name: String) -> Self {
        Self {
            ...
            children: Vec::new(),
            device_ids: Vec::new(),
            power_on_latency_us: 100,
            power_off_latency_us: 50,
            usage_count: 0,
            on_count: 0,
            off_count: 0,
        }
...
impl DevicePower {
    pub fn new(id: u64, name: String) -> Self {
        Self {
            device_id: id,
            name,
            rpm_state: RuntimePmState::Active,
            domain_id: None,
            usage_count: 0,
            child_count: 0,
            disable_depth: 0,
    ...
    #[inline(always)]
    pub fn mark_busy(&mut self, ts: u64) {
        self.last_busy_ts = ts;
        self.rpm_state = RuntimePmState::Active;
    }

    #[inline(always)]
    pub fn suspend(&mut self) -> bool {
        if self.usage_count > 0 || self.disable_depth > 0 {
            return false;
        }
        self.rpm_state = RuntimePmState::Suspended;
    ...
    pub fn get_ref(&mut self) {
        self.usage_count += 1;
        if self.rpm_state == RuntimePmState::Suspended {
            self.resume();
        }
    }
    #[inline(always)]
    ...
    pub fn is_idle(&self, now: u64) -> bool {
        self.usage_count == 0 && now - self.last_busy_ts > self.idle_timeout_ms * 1000
    }
}
...
impl WakeupSource {
    ...
    #[inline(always)]
    pub fn activate(&mut self, ts: u64) {
        self.active = true;
    ...
    pub fn deactivate(&mut self, ts: u64) {
        if self.active {
            ...
            if dur > self.max_time_us {
                self.max_time_us = dur;
            }
        }
        self.active = false;
    }
...
impl BridgePowerBridge {
    pub fn new() -> Self {
        Self {
            ...
            system_state: SleepState::S0,
            stats: PowerBridgeStats::default(),
            next_id: 1,
        }
    }
    ...
    #[inline(always)]
    pub fn device_get(&mut self, id: u64) {
        if let Some(d) = self.devices.get_mut(&id) {
            d.get_ref();
        }
    }
    #[inline(always)]
    pub fn device_put(&mut self, id: u64) {
        if let Some(d) = self.devices.get_mut(&id) {
            d.put_ref();
        }
    }
    #[inline(always)]
    pub fn device_busy(&mut self, id: u64, ts: u64) {
        if let Some(d) = self.devices.get_mut(&id) {
            d.mark_busy(ts);
        }
    }
    ...
    #[inline(always)]
    pub fn suspend_device(&mut self, id: u64) -> bool {
        self.devices
            .get_mut(&id)
            .map(|d| d.suspend())
            .unwrap_or(false)
    ...
    #[inline(always)]
    pub fn activate_wakeup(&mut self, id: u64, ts: u64) {
        if let Some(w) = self.wakeup_sources.get_mut(&id) {
            w.activate(ts);
        }
    }
    #[inline(always)]
    pub fn deactivate_wakeup(&mut self, id: u64, ts: u64) {
        if let Some(w) = self.wakeup_sources.get_mut(&id) {
            w.deactivate(ts);
        }
    }
    ...
    #[inline(always)]
    pub fn system_sleep(&self) -> SleepState {
        self.system_state
    }
    #[inline(always)]
    pub fn set_sleep_state(&mut self, s: SleepState) {
        self.system_state = s;
    }
    ...
    pub fn auto_suspend_idle(&mut self, now: u64) -> Vec<u64> {
        let idle: Vec<u64> = self
            .devices
            .values()
            .filter(|d| d.runtime_auto && d.is_idle(now) && d.rpm_state == RuntimePmState::Active)
            .map(|d| d.device_id)
            .collect();
        for &id in &idle {
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.domains_off = self
            .domains
            .values()
            .filter(|d| d.state == DomainState::Off)
            .count();
        self.stats.total_devices = self.devices.len();
        self.stats.suspended_devices = self
            .devices
            .values()
            .filter(|d| d.rpm_state == RuntimePmState::Suspended)
            .count();
        self.stats.total_wakeup_sources = self.wakeup_sources.len();
        self.stats.active_wakeup_sources =
            self.wakeup_sources.values().filter(|w| w.active).count();
    ...
    #[inline(always)]
    pub fn domain(&self, id: u64) -> Option<&PowerDomain> {
        self.domains.get(&id)
    }
    #[inline(always)]
    pub fn device(&self, id: u64) -> Option<&DevicePower> {
        self.devices.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &PowerBridgeStats {
        &self.stats
    }
}
