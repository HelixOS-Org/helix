...
impl BlockReq {
    ...
    #[inline(always)]
    pub fn dispatch(&mut self, ts: u64) {
        self.state = BioState::Dispatched;
        self.dispatch_ts = ts;
    }
    #[inline(always)]
    pub fn start(&mut self) {
        self.state = BioState::InFlight;
    }
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn fail(&mut self, err: i32, ts: u64) {
    ...
    pub fn queue_latency_ns(&self) -> u64 {
        if self.dispatch_ts > self.enqueue_ts {
            self.dispatch_ts - self.enqueue_ts
        } else {
            0
        }
    }
    ...
    pub fn service_time_ns(&self) -> u64 {
        if self.complete_ts > self.dispatch_ts {
            self.complete_ts - self.dispatch_ts
        } else {
            0
        }
    }
    ...
    pub fn total_latency_ns(&self) -> u64 {
        if self.complete_ts > self.enqueue_ts {
            self.complete_ts - self.enqueue_ts
        } else {
            0
        }
    }
...
impl BlockDevice {
    ...
    pub fn record_complete(&mut self, req: &BlockReq) {
        match req.op {
            BioOp::Read => {
                self.read_ios += 1;
                self.read_bytes += req.bytes();
            },
            BioOp::Write => {
                self.write_ios += 1;
                self.write_bytes += req.bytes();
            },
            BioOp::Discard | BioOp::SecureErase => {
                self.discard_ios += 1;
            },
            BioOp::Flush => {
                self.flush_ios += 1;
...
impl BridgeBlockBridge {
    pub fn new() -> Self {
        Self {
            ...
            stats: BlockBridgeStats::default(),
            next_dev: 1,
            next_req: 1,
        }
    }
    ...
    pub fn register_device(&mut self, name: String, capacity: u64) -> u64 {
        ...
        self.next_dev += 1;
        self.devices
            .insert(id, BlockDevice::new(id, name, capacity));
        id
    }
    ...
    #[inline(always)]
    pub fn set_sched(&mut self, dev: u64, sched: IoSched) {
        if let Some(d) = self.devices.get_mut(&dev) {
            d.sched = sched;
        }
    }
    #[inline(always)]
    pub fn set_queue_depth(&mut self, dev: u64, depth: u32) {
        if let Some(d) = self.devices.get_mut(&dev) {
            d.queue_depth = depth;
        }
    }
    ...
    pub fn recompute(&mut self) {
        ...
        let done: Vec<&BlockReq> = self
            .requests
            .values()
            .filter(|r| r.state == BioState::Completed)
            .collect();
        if !done.is_empty() {
            let total: u64 = done.iter().map(|r| r.total_latency_ns()).sum();
            self.stats.avg_latency_ns = total / done.len() as u64;
            self.stats.max_latency_ns =
                done.iter().map(|r| r.total_latency_ns()).max().unwrap_or(0);
        }
        self.stats.total_errors = self
            .requests
            .values()
            .filter(|r| r.state == BioState::Error)
            .count() as u64;
        self.stats.total_merges = self.requests.values().filter(|r| r.merged).count() as u64;
    }

    #[inline(always)]
    pub fn device(&self, id: u64) -> Option<&BlockDevice> {
        self.devices.get(&id)
    }
    #[inline(always)]
    pub fn request(&self, id: u64) -> Option<&BlockReq> {
        self.requests.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &BlockBridgeStats {
        &self.stats
    }
}
