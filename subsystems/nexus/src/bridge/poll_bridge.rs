...
impl PollEvents {
    ...
    pub fn is_readable(&self) -> bool {
        self.has(Self::POLLIN) || self.has(Self::POLLRDNORM)
    }
    #[inline(always)]
    pub fn is_writable(&self) -> bool {
        self.has(Self::POLLOUT) || self.has(Self::POLLWRNORM)
    }
    #[inline(always)]
    pub fn is_error(&self) -> bool {
        self.has(Self::POLLERR) || self.has(Self::POLLHUP) || self.has(Self::POLLNVAL)
    }
}
...
impl PollFdEntry {
    pub fn new(fd: i32, events: PollEvents) -> Self {
        Self {
            ...
            returned_events: PollEvents::empty(),
            wait_queue_registered: false,
        }
    }

    #[inline(always)]
    pub fn is_ready(&self) -> bool {
        self.returned_events.bits & self.requested_events.bits != 0
            || self.returned_events.is_error()
    }
}
...
impl FdPollStats {
    ...
    pub fn ready_ratio(&self) -> f64 {
        if self.poll_count == 0 {
            ...
            self.ready_count as f64 / self.poll_count as f64
        }
    }
}
...
impl BridgePollBridge {
    pub fn new() -> Self {
        Self {
            ...
            fd_stats: BTreeMap::new(),
            max_requests: 1024,
            next_id: 1,
            stats: PollBridgeStats::default(),
        }
    }
        ...
        if let Some(r) = self.requests.iter_mut().find(|r| r.request_id == req_id) {
            ...
            for fde in &r.fds {
                let fs = self
                    .fd_stats
                    .entry(fde.fd)
                    .or_insert_with(|| FdPollStats::new(fde.fd));
                fs.poll_count += 1;
                fs.total_wait_ns += wait;
                if fde.is_ready() {
                    fs.ready_count += 1;
                }
                fs.avg_wait_ns = if fs.poll_count == 0 {
                    0
                } else {
                    fs.total_wait_ns / fs.poll_count
                };
            }
        }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.select_calls = self
            .requests
            .iter()
            .filter(|r| r.variant == PollVariant::Select)
            .count() as u64;
        self.stats.poll_calls = self
            .requests
            .iter()
            .filter(|r| r.variant == PollVariant::Poll)
            .count() as u64;
        self.stats.ppoll_calls = self
            .requests
            .iter()
            .filter(|r| r.variant == PollVariant::Ppoll)
            .count() as u64;
        let fd_counts: Vec<f64> = self.requests.iter().map(|r| r.fd_count() as f64).collect();
        self.stats.avg_fd_count = if fd_counts.is_empty() {
            0.0
        } else {
            fd_counts.iter().sum::<f64>() / fd_counts.len() as f64
        };
        self.stats.max_fd_count = self
            .requests
            .iter()
            .map(|r| r.fd_count())
            .max()
        ...
        self.stats.timeout_ratio = if total <= 0.0 {
            0.0
        } else {
            self.requests.iter().filter(|r| r.timed_out).count() as f64 / total
        };
        self.stats.total_fds_tracked = self.fd_stats.len();
    }

    #[inline(always)]
    pub fn fd_stats(&self, fd: i32) -> Option<&FdPollStats> {
        self.fd_stats.get(&fd)
    }
    #[inline(always)]
    pub fn stats(&self) -> &PollBridgeStats {
        &self.stats
    }
}
...
impl BridgePollV2 {
    ...
    pub fn record_call(&mut self, call: &PollV2Call) {
        ...
        if self.stats.total_calls > 0 {
            self.stats.avg_fds_per_call =
                self.stats.total_fds_polled as f64 / self.stats.total_calls as f64;
        }
    }
...
impl PollV3Record {
    pub fn new(event: PollV3Event, fd: i32) -> Self {
        Self {
            event,
            fd,
            nfds: 0,
...
/// Main bridge poll v3
#[derive(Debug)]
pub struct BridgePollV3 {
    pub stats: PollV3BridgeStats,
}

impl BridgePollV3 {
    pub fn new() -> Self {
        Self {
            stats: PollV3BridgeStats {
                total_polls: 0,
                ready_events: 0,
                errors: 0,
                timeouts: 0,
