...
impl PipeInstance {
    ...
    pub fn capacity_bytes(&self) -> u64 {
        self.capacity_pages as u64 * 4096
    }
    ...
    #[inline(always)]
    pub fn set_capacity(&mut self, pages: u32) {
    ...
    pub fn fill_ratio(&self) -> f64 {
        ...
        if cap == 0 {
            ...
            self.bytes_buffered as f64 / cap as f64
        }
    }
    ...
    pub fn throughput_bps(&self, current_ts: u64) -> f64 {
        ...
        if elapsed == 0 {
            ...
            self.total_bytes_written as f64 / (elapsed as f64 / 1_000_000_000.0)
        }
    }
}
...
impl BridgePipeBridge {
    pub fn new() -> Self {
        Self {
            pipes: BTreeMap::new(),
            splice_history: VecDeque::new(),
            max_splice_history: 512,
            next_id: 1,
    ...
    pub fn create_pipe(&mut self, reader: u64, writer: u64, ts: u64) -> u64 {
        ...
        self.next_id += 1;
        self.pipes
            .insert(id, PipeInstance::new(id, reader, writer, ts));
        id
    }

    #[inline(always)]
    pub fn write(&mut self, pipe_id: u64, bytes: u64, ts: u64) -> bool {
        if let Some(p) = self.pipes.get_mut(&pipe_id) {
            p.write(bytes, ts)
        } else {
            false
        }
    ...
    pub fn splice(&mut self, src_fd: i32, dst_fd: i32, bytes: u64, flags: u32, ts: u64) {
        self.splice_history.push_back(SpliceRecord {
            src_fd,
            dst_fd,
            bytes,
            flags,
            timestamp: ts,
            zero_copy: flags & 0x4 != 0,
        });
        if self.splice_history.len() > self.max_splice_history {
            self.splice_history.remove(0);
        }
    }
    ...
    pub fn set_capacity(&mut self, pipe_id: u64, pages: u32) {
        if let Some(p) = self.pipes.get_mut(&pipe_id) {
            p.set_capacity(pages);
        }
    }

    pub fn recompute(&mut self) {
        self.stats.total_pipes = self.pipes.len();
        self.stats.active_pipes = self
            .pipes
            .values()
            .filter(|p| p.state == PipeState::Open)
            .count();
        self.stats.broken_pipes = self
            .pipes
            .values()
            .filter(|p| p.state == PipeState::Broken)
            .count();
        self.stats.total_bytes_transferred =
            self.pipes.values().map(|p| p.total_bytes_written).sum();
        ...
        let fills: Vec<f64> = self
            ...
            .map(|p| p.fill_ratio())
            .collect();
        self.stats.avg_fill_ratio = if fills.is_empty() {
            0.0
        } else {
    ...
    #[inline(always)]
    pub fn pipe(&self, id: u64) -> Option<&PipeInstance> {
        self.pipes.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &PipeBridgeStats {
        &self.stats
    }
}
