...
impl DmaFence {
    ...
    #[inline(always)]
    pub fn signal(&mut self, ts: u64) {
        self.state = FenceState::Signaled;
        self.signal_ts = ts;
    }
    #[inline(always)]
    pub fn signal_error(&mut self, err: i32, ts: u64) {
    ...
    pub fn latency_ns(&self) -> u64 {
        if self.signal_ts > self.create_ts {
            self.signal_ts - self.create_ts
        } else {
            0
        }
    }
    ...
    #[inline(always)]
    pub fn add_waiter(&mut self, waiter: u64) {
        if !self.waiters.contains(&waiter) {
            self.waiters.push(waiter);
        }
    }
    #[inline(always)]
    pub fn set_deadline(&mut self, ts: u64) {
        self.deadline_ts = ts;
    }
    #[inline(always)]
    pub fn is_past_deadline(&self, now: u64) -> bool {
        self.deadline_ts > 0 && now > self.deadline_ts && !self.is_signaled()
    }
}
...
impl BridgeDmaFence {
    ...
    pub fn signal_fence(&mut self, id: u64, ts: u64) {
        if let Some(f) = self.fences.get_mut(&id) {
            ...
            if let Some(c) = self.contexts.get_mut(&ctx) {
                ...
                c.total_signaled += 1;
                if seqno > c.last_signaled_seqno {
                    c.last_signaled_seqno = seqno;
                }
                let lat = f.latency_ns();
                c.avg_latency_ns =
                    (c.avg_latency_ns * (c.total_signaled - 1) + lat) / c.total_signaled;
            }
        }
    ...
    pub fn add_dependency(&mut self, fence: u64, dep: u64) {
        if let Some(f) = self.fences.get_mut(&fence) {
            f.add_dep(dep);
        }
    }
    ...
    pub fn wait_fence(&mut self, fence_id: u64, waiter: u64, timeout: u64, ts: u64) {
        if let Some(f) = self.fences.get_mut(&fence_id) {
            f.add_waiter(waiter);
        }
        self.waits.push(FenceWaitReq {
            fence_id,
            waiter_id: waiter,
            timeout_ns: timeout,
            start_ts: ts,
    ...
    pub fn check_deadlines(&mut self, now: u64) -> Vec<u64> {
        self.fences
            .values()
            .filter(|f| f.is_past_deadline(now))
            .map(|f| f.id)
            .collect()
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.signaled_fences = self
            .fences
            .values()
            .filter(|f| f.state == FenceState::Signaled)
            .count() as u64;
        self.stats.error_fences = self
            .fences
            .values()
            .filter(|f| f.state == FenceState::Error)
            .count() as u64;
        let signaled: Vec<u64> = self
            .fences
            .values()
    ...
    #[inline(always)]
    pub fn fence(&self, id: u64) -> Option<&DmaFence> {
        self.fences.get(&id)
    }
    #[inline(always)]
    pub fn context(&self, id: u64) -> Option<&FenceContext> {
