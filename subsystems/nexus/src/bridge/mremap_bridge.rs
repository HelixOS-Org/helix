...
impl BridgeMremapManager {
    ...
    pub fn mremap(&mut self, old_addr: u64, old_size: u64, new_size: u64, may_move: bool) -> u64 {
        ...
        let (new_addr, moved) = if may_move && new_size > old_size {
            ...
            self.next_addr += (new_size + 4095) & !4095;
            self.stats.moves += 1;
            (a, true)
        } else {
        ...
        self.history.push(BridgeMremapRecord {
            old_addr,
            old_size,
            new_addr,
            new_size,
    ...
    #[inline(always)]
    pub fn stats(&self) -> &BridgeMremapStats {
        &self.stats
    }
}
