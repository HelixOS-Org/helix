...
impl WorkItem {
    ...
    #[inline(always)]
    pub fn set_delayed(&mut self, delay: u64) {
        self.delay_ns = delay;
        self.state = WorkState::Delayed;
    }
...
impl Workqueue {
    ...
    #[inline(always)]
    pub fn add_flag(&mut self, f: WqFlag) {
    ...
    pub fn is_unbound(&self) -> bool {
        self.flags.contains(&WqFlag::Unbound)
    }
    #[inline(always)]
    pub fn is_high_pri(&self) -> bool {
        self.flags.contains(&WqFlag::HighPri)
    }
    #[inline(always)]
    pub fn can_schedule(&self) -> bool {
        self.current_active < self.max_active
    }
    #[inline(always)]
    pub fn utilization(&self) -> f64 {
        if self.max_active == 0 {
            ...
            self.current_active as f64 / self.max_active as f64 * 100.0
        }
    }
}
...
impl BridgeWqProxy {
    ...
    pub fn queue_work(&mut self, wq: u64, handler: u64, prio: WorkPriority, ts: u64) -> u64 {
        ...
        if let Some(q) = self.queues.get_mut(&wq) {
            q.nr_pending += 1;
        }
        id
    }
    ...
    pub fn start_work(&mut self, item_id: u64, ts: u64) {
        if let Some(w) = self.items.get_mut(&item_id) {
            ...
            w.start(ts);
            if let Some(q) = self.queues.get_mut(&wq) {
                q.nr_pending = q.nr_pending.saturating_sub(1);
                q.current_active += 1;
            }
        }
    }
    ...
    pub fn complete_work(&mut self, item_id: u64, ts: u64) {
        if let Some(w) = self.items.get_mut(&item_id) {
            ...
            w.complete(ts);
            if let Some(q) = self.queues.get_mut(&wq) {
                q.current_active = q.current_active.saturating_sub(1);
                q.total_executed += 1;
            }
        }
    }
    ...
    #[inline(always)]
    pub fn add_pool(&mut self, cpu: u32) {
        self.pools.insert(cpu, WorkerPool::new(cpu));
    }
    ...
    pub fn recompute(&mut self) {
        ...
        let done: Vec<&WorkItem> = self
            .items
            .values()
            .filter(|w| w.state == WorkState::Done)
            .collect();
        if !done.is_empty() {
    ...
    #[inline(always)]
    pub fn wq(&self, id: u64) -> Option<&Workqueue> {
        self.queues.get(&id)
    }
    #[inline(always)]
    pub fn work(&self, id: u64) -> Option<&WorkItem> {
        self.items.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &WqProxyStats {
        &self.stats
    }
}
