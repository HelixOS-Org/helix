...
impl BridgeAudit {
    ...
    pub fn log(
        ...
        if self.records.len() as u32 >= self.backlog_limit {
            self.dropped += 1;
            return 0;
        }
        let seq = self.next_seq;
        self.next_seq += 1;
        self.records.insert(seq, AuditRecord {
            seq,
            msg_type,
            pid,
            uid,
...
impl BridgeAuditV2 {
    ...
    pub fn record(&mut self, rec: &AuditV2Record) {
        ...
        if rec.result == AuditV2Result::BufferFull {
            self.stats.buffer_overflows += 1;
        }
        if rec.result == AuditV2Result::Error {
            self.stats.errors += 1;
        }
    }
}
