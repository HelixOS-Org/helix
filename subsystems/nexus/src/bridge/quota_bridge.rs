...
impl DiskQuota {
    ...
    pub fn check_block_alloc(&self, additional: u64) -> QuotaEnforcement {
        ...
        if new_total > self.blocks_hard {
            return QuotaEnforcement::Hard;
        }
        if new_total > self.blocks_soft {
            return QuotaEnforcement::Soft;
        }
    ...
    pub fn block_utilization(&self) -> f64 {
        if self.blocks_hard == u64::MAX || self.blocks_hard == 0 {
            return 0.0;
        }
    ...
    pub fn inode_utilization(&self) -> f64 {
        if self.inodes_hard == u64::MAX || self.inodes_hard == 0 {
            return 0.0;
        }
...
impl BridgeQuota {
    ...
    pub fn set_block_limits(&mut self, id: u64, soft: u64, hard: u64) {
        if let Some(q) = self.quotas.get_mut(&id) {
            q.set_block_limits(soft, hard);
        }
    }
    ...
    pub fn check_alloc(&mut self, id: u64, blocks: u64, now: u64) -> QuotaEnforcement {
        let result = self
            .quotas
            .get(&id)
            .map(|q| q.check_block_alloc(blocks))
            .unwrap_or(QuotaEnforcement::None);
        if result != QuotaEnforcement::None {
            if let Some(q) = self.quotas.get(&id) {
                if self.violations.len() >= self.max_violations {
                    self.violations.drain(..self.max_violations / 4);
                }
                self.violations.push(QuotaViolation {
                    quota_id: id,
                    qtype: q.qtype,
                    enforcement: result,
                    resource: QuotaResource::Blocks,
                    amount: blocks,
                    timestamp: now,
                });
            }
    ...
    pub fn stats(&self) -> QuotaBridgeStats {
        let hard = self
            .violations
            .iter()
            .filter(|v| v.enforcement == QuotaEnforcement::Hard)
            .count() as u64;
        let over_soft = self
            .quotas
            .values()
            .filter(|q| q.blocks_used > q.blocks_soft || q.inodes_used > q.inodes_soft)
            .count() as u32;
        QuotaBridgeStats {
...
impl QuotaV2Usage {
    ...
    #[inline(always)]
    pub fn block_usage_ratio(&self) -> f64 {
        if self.limits.block_hard == 0 {
            0.0
        } else {
            self.blocks_used as f64 / self.limits.block_hard as f64
        }
    }
    #[inline(always)]
    pub fn inode_usage_ratio(&self) -> f64 {
        if self.limits.inode_hard == 0 {
            0.0
        } else {
            self.inodes_used as f64 / self.limits.inode_hard as f64
        }
    }
    #[inline(always)]
    pub fn over_soft_block(&self) -> bool {
        self.blocks_used > self.limits.block_soft
    }
    #[inline(always)]
    pub fn over_hard_block(&self) -> bool {
        self.blocks_used > self.limits.block_hard
    }
}
...
impl BridgeQuotaV2 {
    ...
    pub fn create(&mut self, qt: QuotaV2Type, owner: u32, limits: QuotaV2Limits) -> u64 {
        ...
        self.next_id += 1;
        self.quotas
            .insert(id, QuotaV2Usage::new(id, qt, owner, limits));
        id
    }
    ...
    pub fn stats(&self) -> QuotaV2BridgeStats {
        let user = self
            .quotas
            .values()
            .filter(|q| q.quota_type == QuotaV2Type::User)
            .count() as u32;
        let group = self
            .quotas
            .values()
            .filter(|q| q.quota_type == QuotaV2Type::Group)
            .count() as u32;
