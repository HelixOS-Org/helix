...
impl BridgeSysfsProxy {
    ...
    pub fn create_kobject(
        ...
        if let Some(pid) = parent {
            if let Some(p) = self.kobjects.get_mut(&pid) {
                p.children.push(id);
            }
        }
        id
    ...
    pub fn emit_uevent(&mut self, kobj_id: u64, action: UeventAction, ts: u64) {
        ...
        if let Some(ss) = subsys {
            ...
            let filtered = self.filters.iter().any(|f| {
                (f.subsystem.is_none() || f.subsystem == Some(ss))
                    && (f.action.is_none() || f.action == Some(action))
            });
            if !filtered {
                self.uevents.push(ev);
            } else {
                self.stats.filtered_uevents += 1;
            }
        }
    }
    ...
    pub fn add_filter(&mut self, subsys: Option<SysfsSubsystem>, action: Option<UeventAction>) {
        self.filters.push(UeventFilter {
            subsystem: subsys,
            action,
            match_count: 0,
        });
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.total_reads = self
            .kobjects
            .values()
            .flat_map(|k| k.attrs.iter())
            .map(|a| a.read_count)
            .sum();
        self.stats.total_writes = self
            .kobjects
            .values()
            .flat_map(|k| k.attrs.iter())
            .map(|a| a.write_count)
    ...
    #[inline(always)]
    pub fn kobject(&self, id: u64) -> Option<&KObject> {
        self.kobjects.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &SysfsProxyStats {
        &self.stats
    }
}
