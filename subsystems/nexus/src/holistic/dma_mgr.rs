...
impl HolisticDmaMgr {
    ...
    pub fn init_zone(&mut self, zone: DmaZone, total_pages: u64) {
        self.zones
            .insert(zone as u8, DmaZoneState::new(zone, total_pages));
    }
    ...
    pub fn alloc_buffer(
        ...
        self.buffers.insert(
            id,
            DmaBuffer::new(id, phys, virt, dma, size, zone, coh, owner, ts),
        );
    ...
    pub fn free_buffer(&mut self, id: u64) {
        if let Some(buf) = self.buffers.remove(&id) {
            ...
            if let Some(z) = self.zones.get_mut(&(buf.zone as u8)) {
                z.free(pages);
            }
    ...
    pub fn add_sg_entry(&mut self, sg_id: u64, phys: u64, len: u32, offset: u32) {
        if let Some(sg) = self.sg_lists.get_mut(&sg_id) {
            sg.add_entry(phys, len, offset);
        }
    }
    ...
    pub fn start_transfer(&mut self, buf_id: u64, dir: DmaDirection, device: u64, ts: u64) -> u64 {
        ...
        self.transfers.push(DmaTransfer {
            ...
            end_ts: 0,
            device_id: device,
            status: DmaTransferStatus::InProgress,
        });
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.errors = self
            .transfers
            .iter()
            .filter(|x| x.status == DmaTransferStatus::Error)
            .count() as u64;
    }

    #[inline(always)]
    pub fn buffer(&self, id: u64) -> Option<&DmaBuffer> {
        self.buffers.get(&id)
    }
    #[inline(always)]
    pub fn sg(&self, id: u64) -> Option<&SgList> {
        self.sg_lists.get(&id)
    }
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn zone(&self, z: DmaZone) -> Option<&DmaZoneState> {
        self.zones.get(&(z as u8))
    }
}
