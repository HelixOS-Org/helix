...
impl LatencyHolisticRecord {
    pub fn new(bucket: LatencyBucket, latency_us: u64) -> Self {
        Self {
            bucket,
            latency_us,
            p99_us: 0,
...
#[derive(Debug, Clone)]
#[repr(align(64))]
pub struct LatencyHolisticStats {
...
impl HolisticLatency {
    ...
    pub fn record(&mut self, rec: &LatencyHolisticRecord) {
        ...
        if rec.bucket == LatencyBucket::Over100ms || rec.bucket == LatencyBucket::Over1s {
            self.stats.over_100ms += 1;
        }
        if rec.latency_us > self.stats.worst_us {
            self.stats.worst_us = rec.latency_us;
        }
