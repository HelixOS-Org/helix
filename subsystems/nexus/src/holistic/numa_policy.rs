...
impl HolisticNumaPolicy {
    ...
    pub fn add_node(&mut self, node_id: u32, total_pages: u64) {
        self.nodes
            .insert(node_id, NodeMemInfo::new(node_id, total_pages));
    }
    ...
    #[inline(always)]
    pub fn unbind_process(&mut self, pid: u64) -> bool {
        ...
        match binding.policy {
            ...
            NumaPolicy::Interleave => {
                ...
                if allowed.is_empty() {
                    ...
                    Some(
                        allowed
                            [(binding.pages_local + binding.pages_remote) as usize % allowed.len()],
                    )
                }
            },
    ...
    pub fn record_migration(
        ...
        if let Some(b) = self.bindings.get_mut(&pid) {
            b.pages_migrated += pages;
        }
        if self.events.len() >= self.max_events {
            self.events.drain(..self.max_events / 4);
        }
        self.events.push(NumaBalanceEvent {
            pid,
            from_node: from,
            to_node: to,
            pages,
    ...
    pub fn stats(&self) -> NumaPolicyStats {
        ...
        let avg_loc = if localities.is_empty() {
            1.0
        } else {
            localities.iter().sum::<f64>() / localities.len() as f64
        };
        let mut node_pressure = ArrayMap::new(0.0);
        for (&id, info) in &self.nodes {
            node_pressure.insert(id, info.pressure());
        }
        NumaPolicyStats {
