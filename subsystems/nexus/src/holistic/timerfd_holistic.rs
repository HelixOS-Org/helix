...
impl TimerfdHolisticRecord {
    pub fn new(precision: TimerPrecision) -> Self {
        Self {
            precision,
            interval_ns: 0,
            jitter_ns: 0,
...
#[derive(Debug, Clone)]
#[repr(align(64))]
pub struct TimerfdHolisticStats {
    pub total_samples: u64,
    pub coarse_timers: u64,
...
#[derive(Debug)]
#[repr(align(64))]
pub struct HolisticTimerfd {
    pub stats: TimerfdHolisticStats,
}

impl HolisticTimerfd {
    pub fn new() -> Self {
        Self {
            stats: TimerfdHolisticStats {
                total_samples: 0,
                coarse_timers: 0,
                total_overruns: 0,
                max_jitter_ns: 0,
    ...
    pub fn record(&mut self, rec: &TimerfdHolisticRecord) {
        ...
        if rec.precision == TimerPrecision::Coarse {
            self.stats.coarse_timers += 1;
        }
        self.stats.total_overruns += rec.overruns as u64;
        if rec.jitter_ns > self.stats.max_jitter_ns {
            self.stats.max_jitter_ns = rec.jitter_ns;
        }
    }
}
