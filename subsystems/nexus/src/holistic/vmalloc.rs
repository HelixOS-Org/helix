...
impl HolisticVmalloc {
    pub fn new(space: u64) -> Self {
        let holes = alloc::vec![VmallocHole {
            addr: 0,
            size: space
        }];
        Self {
    ...
    pub fn alloc(
        ...
        for i in 0..self.free_holes.len() {
            if self.free_holes[i].size >= aligned {
                ...
                if self.free_holes[i].size == 0 {
                    self.free_holes.remove(i);
                }
                self.areas
                    .insert(addr, VmallocArea::new(addr, aligned, atype, caller, now));
                self.total_allocs += 1;
                return Some(addr);
            }
        }
    ...
    pub fn free(&mut self, addr: u64) {
        if let Some(area) = self.areas.remove(&addr) {
            self.free_holes.push(VmallocHole {
                addr: area.addr,
                size: area.size,
            });
            self.total_frees += 1;
