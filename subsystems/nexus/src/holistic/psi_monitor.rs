...
impl PsiTrigger {
    ...
    pub fn should_fire(&self, reading: &PsiReading, now: u64) -> bool {
        ...
        if now.saturating_sub(self.last_fired) < self.cooldown_us {
            return false;
        }
...
impl CgroupPsi {
    ...
    pub fn overall_pressure(&self) -> f64 {
        let vals = [
            self.cpu.max_pressure(),
            self.memory.max_pressure(),
            self.io.max_pressure(),
        ];
        vals.iter().cloned().fold(0.0_f64, f64::max)
    }
...
impl HolisticPsiMonitor {
    ...
    pub fn update_cgroup_psi(
        ...
        let entry = self.cgroup_psi.entry(cgroup_id).or_insert_with(|| {
    ...
    pub fn check_triggers(&mut self, now: u64) -> Vec<PsiAlert> {
        ...
        for trigger in self.triggers.values_mut() {
            ...
            for reading in &readings {
                if reading.resource != trigger.resource {
                    continue;
                }
