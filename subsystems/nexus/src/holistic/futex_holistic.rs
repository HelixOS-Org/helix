...
impl FutexHolisticRecord {
    pub fn new(health: FutexQueueHealth) -> Self {
        Self {
            health,
            futex_addr: 0,
            queue_depth: 0,
...
#[derive(Debug, Clone)]
#[repr(align(64))]
pub struct FutexHolisticStats {
    pub total_samples: u64,
    pub thundering_herds: u64,
    pub starved: u64,
    pub peak_depth: u32,
}
...
/// Main holistic futex
#[derive(Debug)]
pub struct HolisticFutex {
    pub stats: FutexHolisticStats,
}

impl HolisticFutex {
    pub fn new() -> Self {
        Self {
            stats: FutexHolisticStats {
                total_samples: 0,
                thundering_herds: 0,
                starved: 0,
                peak_depth: 0,
    ...
    pub fn record(&mut self, rec: &FutexHolisticRecord) {
        ...
        if rec.queue_depth > self.stats.peak_depth {
            self.stats.peak_depth = rec.queue_depth;
        }
    }
}
