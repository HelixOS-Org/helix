...
impl BuddyZone {
    ...
    pub fn add_free(&mut self, pfn: u64, order: u8) {
        if (order as usize) < MAX_ORDER {
            self.free_lists[order as usize].push(FreeBlock { pfn, order });
        }
    }
        ...
        for o in (order as usize)..MAX_ORDER {
            if let Some(block) = self.free_lists[o].pop() {
                ...
                while current_order > order as usize {
                    ...
                    self.free_lists[current_order].push(FreeBlock {
                        pfn: buddy_pfn,
                        order: current_order as u8,
                    });
                    self.split_count += 1;
                }
...
impl HolisticBuddyAlloc {
    ...
    pub fn alloc(&mut self, order: u8) -> Option<u64> {
        for zone in &mut self.zones {
            if let Some(pfn) = zone.alloc(order) {
                return Some(pfn);
            }
        }
        None
...
impl HolisticBuddyAllocV2 {
    pub fn new(total: u64) -> Self {
        Self {
            zone: BuddyV2Zone::new(total),
        }
    }
    #[inline(always)]
    pub fn alloc(&mut self, order: u32) -> Option<u64> {
        self.zone.alloc(order)
    }
    #[inline(always)]
    pub fn free(&mut self, pfn: u64, order: u32) {
        self.zone.free(pfn, order)
    }

    #[inline(always)]
    pub fn stats(&self) -> BuddyAllocV2Stats {
        BuddyAllocV2Stats {
            total_pages: self.zone.total_pages,
            free_pages: self.zone.free_pages,
            allocs: self.zone.alloc_count,
            frees: self.zone.free_count,
            splits: self.zone.split_count,
            merges: self.zone.merge_count,
        }
    }
}
