...
impl Vma {
    ...
    pub fn can_merge_with(&self, other: &Vma) -> bool {
        self.end == other.start
            && self.perms.to_bits() == other.perms.to_bits()
    ...
    pub fn thp_score(&self) -> f64 {
        ...
        if self.vma_type == VmaType::Anonymous {
            score += 0.2;
        }
        // High RSS ratio suggests active usage
        if self.rss_ratio() > 0.8 {
            score += 0.1;
        }
        score
    }
...
impl ProcessAddressSpace {
        ...
        for i in 1..self.vmas.len() {
            if self.vmas[i].start > self.vmas[i - 1].end {
                gaps.push(AddressGap {
                    start: self.vmas[i - 1].end,
                    end: self.vmas[i].start,
                });
            }
        }
        gaps
    ...
    pub fn recompute(&mut self) {
        ...
        if self.vmas.len() >= 2 {
            ...
            self.fragmentation_score = if span == 0 {
                0.0
            } else {
                gap_total as f64 / span as f64
            };
        }
    }
}
...
impl HolisticMmapAdvisor {
    pub fn new() -> Self {
        Self {
            ...
            stats: MmapAdvisorStats::default(),
        }
    }
    ...
    pub fn madvise(&mut self, pid: u64, start: u64, len: u64, hint: MadviseHint) {
        if let Some(p) = self.processes.get_mut(&pid) {
            p.apply_madvise(start, len, hint);
        }
    }
    ...
    pub fn recompute(&mut self) {
        ...
        let frags: Vec<f64> = self
            .processes
            .values()
            .map(|p| p.fragmentation_score)
            .collect();
        self.stats.avg_fragmentation = if frags.is_empty() {
            0.0
        } else {
            frags.iter().sum::<f64>() / frags.len() as f64
        };
        self.stats.thp_eligible_vmas = self
            .processes
            .values()
            .flat_map(|p| p.vmas.iter())
            .filter(|v| v.thp_eligible)
            .count();
        self.stats.ksm_mergeable_vmas = self
            .processes
            .values()
            .flat_map(|p| p.vmas.iter())
            .filter(|v| v.ksm_mergeable)
            .count();
    }

    #[inline(always)]
    pub fn process(&self, pid: u64) -> Option<&ProcessAddressSpace> {
        self.processes.get(&pid)
    }
    #[inline(always)]
    pub fn stats(&self) -> &MmapAdvisorStats {
        &self.stats
    }
}
