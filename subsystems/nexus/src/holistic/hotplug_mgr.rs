...
impl HolisticHotplugMgr {
    ...
    pub fn register_notifier(
        ...
        self.notifiers
            .insert(id, HotplugNotifier::new(id, resource, priority, now));
        id
    }

    #[inline(always)]
    pub fn unregister_notifier(&mut self, id: u64) -> bool {
    ...
    pub fn begin_operation(
        ...
        if self.operations.len() >= self.max_operations {
            self.operations.drain(..self.max_operations / 4);
        }
        self.operations
            .push(HotplugOperation::new(id, resource, action, res_id, now));
        id
    }
    ...
    pub fn track_cpu(&mut self, cpu_id: u32, online: bool) {
        self.cpu_states
            .entry(cpu_id)
            .or_insert_with(|| CpuHotplugState::new(cpu_id, online));
    }

    #[inline(always)]
    pub fn cpu_online(&mut self, cpu_id: u32, now: u64) {
        if let Some(state) = self.cpu_states.get_mut(&cpu_id) {
            state.transition(true, now);
        }
    }
    ...
    pub fn stats(&self) -> HotplugMgrStats {
        let successful = self
            .operations
            .iter()
            .filter(|o| o.state == HotplugState::Done)
            .count() as u64;
        let failed = self
            .operations
            .iter()
            .filter(|o| o.state == HotplugState::Failed)
            .count() as u64;
        let rollback = self
            .operations
            .iter()
            .filter(|o| o.state == HotplugState::RollingBack)
            .count() as u64;
        let online = self.cpu_states.values().filter(|c| c.online).count() as u32;
        let latencies: Vec<u64> = self
            .operations
            .iter()
            .filter(|o| o.completed_at > 0)
            .map(|o| o.latency_ns())
            .collect();
        ...
        HotplugMgrStats {
            ...
            failed_ops: failed,
            rollback_ops: rollback,
            active_notifiers: self.notifiers.len() as u32,
            online_cpus: online,
