...
impl HolisticMemcgOom {
    ...
    #[inline(always)]
    pub fn register_cgroup(&mut self, id: u64) {
        self.cgroups.insert(id, CgroupOomState::new(id));
    }
    ...
    pub fn select_victim(&self, candidates: &[OomVictim]) -> Option<u64> {
        candidates
            .iter()
            .filter(|v| !v.is_unkillable)
            .max_by_key(|v| v.effective_score())
            .map(|v| v.pid)
    }
    ...
    pub fn oom_kill(&mut self, cgroup_id: u64, victim_pid: u64, freed: u64, now: u64) {
        if let Some(cg) = self.cgroups.get_mut(&cgroup_id) {
            cg.oom_count += 1;
            cg.last_oom_time = now;
            cg.total_killed += 1;
            cg.total_freed_bytes += freed;
        }
