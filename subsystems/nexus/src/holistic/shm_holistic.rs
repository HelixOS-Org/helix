...
impl ShmHolisticRecord {
    pub fn new(utilization: ShmUtilization) -> Self {
        Self {
            utilization,
            shmid: -1,
            pages_active: 0,
...
/// Main holistic shm
#[derive(Debug)]
pub struct HolisticShm {
    pub stats: ShmHolisticStats,
}
...
impl HolisticShm {
    ...
    pub fn record(&mut self, rec: &ShmHolisticRecord) {
        ...
        if rec.pages_dirty > self.stats.peak_dirty {
            self.stats.peak_dirty = rec.pages_dirty;
        }
    }
}
