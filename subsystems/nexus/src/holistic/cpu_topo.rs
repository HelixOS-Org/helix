...
impl CpuFeatures {
    ...
    #[inline(always)]
    pub fn set(&mut self, feat: u32) {
        if feat < 256 {
            self.bits[feat as usize / 64] |= 1 << (feat % 64);
        }
    }
    #[inline(always)]
    pub fn has(&self, feat: u32) -> bool {
        if feat >= 256 {
            ...
            (self.bits[feat as usize / 64] >> (feat % 64)) & 1 != 0
        }
    }
}
...
impl LogicalCpu {
    ...
    #[inline(always)]
    pub fn is_smt_primary(&self) -> bool {
        self.smt_sibling.is_some() && self.cpu_id < self.smt_sibling.unwrap()
    }
    #[inline(always)]
...
impl TopologyDistances {
    ...
    pub fn distance(&self, cpu_a: &LogicalCpu, cpu_b: &LogicalCpu) -> u32 {
        if cpu_a.cpu_id == cpu_b.cpu_id {
            return 0;
        }
        if cpu_a.core_id == cpu_b.core_id && cpu_a.package_id == cpu_b.package_id {
            return self.same_core;
        }
        if cpu_a.die_id == cpu_b.die_id && cpu_a.package_id == cpu_b.package_id {
            return self.same_die;
        }
        if cpu_a.package_id == cpu_b.package_id {
            return self.same_package;
        }
        if cpu_a.numa_node == cpu_b.numa_node {
...
impl HolisticCpuTopoV2 {
    pub fn new() -> Self {
        Self {
            cpus: BTreeMap::new(),
            distances: TopologyDistances::default_x86(),
        }
    }

    #[inline(always)]
    pub fn add_cpu(&mut self, cpu: LogicalCpu) {
        self.cpus.insert(cpu.cpu_id, cpu);
    }

    #[inline(always)]
    pub fn get_cpu(&self, id: u32) -> Option<&LogicalCpu> {
        self.cpus.get(&id)
    }
    ...
    pub fn distance(&self, a: u32, b: u32) -> u32 {
        let cpu_a = match self.cpus.get(&a) {
            Some(c) => c,
            None => return u32::MAX,
        };
        let cpu_b = match self.cpus.get(&b) {
            Some(c) => c,
            None => return u32::MAX,
        };
    ...
    pub fn stats(&self) -> CpuTopoV2Stats {
        ...
        let l3: u64 = self
            .cpus
            .values()
            .map(|c| c.cache_size(CacheLevel::L3) as u64)
            .sum();
        CpuTopoV2Stats {
            total_cpus: self.cpus.len() as u32,
            online_cpus: online,
            total_cores: unique_cores.len() as u32,
            total_packages: pkgs.len() as u32,
            total_numa_nodes: nodes.len() as u32,
            smt_enabled: smt,
            total_l3_kb: l3,
        }
    }
