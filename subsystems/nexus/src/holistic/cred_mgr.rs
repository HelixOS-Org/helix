...
impl CapSet {
    ...
    pub fn has_privileged(&self) -> bool {
        self.has(Capability::CapSysAdmin)
            || self.has(Capability::CapNetAdmin)
            || self.has(Capability::CapSysRawio)
    }
}
...
impl ProcessCred {
    ...
    pub fn is_privileged(&self) -> bool {
        self.is_root() || self.cap_effective.has_privileged()
    }
    ...
    pub fn inherit_to_child(&self) -> ProcessCred {
        ...
        let new_permitted = self
            .cap_inheritable
            .intersect(&self.cap_bounding)
            .union(&self.cap_ambient);
        let new_effective = if self.is_root() {
            new_permitted
        } else {
            self.cap_ambient
        };
...
impl HolisticCredManager {
    pub fn new() -> Self {
        Self {
            ...
            events: Vec::new(),
            max_events: 2048,
            stats: CredManagerStats::default(),
        }
    }
    ...
    pub fn setuid(&mut self, pid: u64, uid: u32, ts: u64) -> bool {
        if let Some(c) = self.creds.get_mut(&pid) {
            ...
            if c.setuid(uid) {
                let etype = if !was_root && uid == 0 {
                    CredEventType::PrivilegeEscalation
                } else if was_root && uid != 0 {
                    CredEventType::PrivilegeDrop
                } else {
                    CredEventType::UidChange
                };
                self.record_event(
                    pid,
                    etype,
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.total_uid_changes = self
            .events
            .iter()
            .filter(|e| e.event_type == CredEventType::UidChange)
            .count() as u64;
        self.stats.total_cap_grants = self
            .events
            .iter()
            .filter(|e| e.event_type == CredEventType::CapGrant)
            .count() as u64;
        self.stats.escalation_attempts = self
            .events
            .iter()
            .filter(|e| e.event_type == CredEventType::PrivilegeEscalation)
            .count() as u64;
        self.stats.events_recorded = self.events.len();
    }

    #[inline(always)]
    pub fn cred(&self, pid: u64) -> Option<&ProcessCred> {
        self.creds.get(&pid)
    }
    #[inline(always)]
    pub fn stats(&self) -> &CredManagerStats {
        &self.stats
    }
    #[inline(always)]
    pub fn events(&self) -> &[CredEvent] {
        &self.events
    }
}
