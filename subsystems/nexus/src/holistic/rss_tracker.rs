...
impl ProcessRss {
    pub fn new(pid: u32) -> Self {
        Self {
            ...
            swapped_pages: 0,
            pgtable_pages: 0,
            soft_limit_pages: None,
            hard_limit_pages: None,
            peak_rss_pages: 0,
            growth_rate_pages_per_sec: 0.0,
            last_sample_ts: 0,
            prev_total: 0,
            oom_score: 0,
    ...
    pub fn update(&mut self, anon: u64, file: u64, shmem: u64, swap: u64, pt: u64, ts: u64) {
        ...
        if total > self.peak_rss_pages {
            self.peak_rss_pages = total;
        }
    ...
    pub fn exceeds_soft_limit(&self) -> bool {
        self.soft_limit_pages
            .map(|l| self.total_rss() > l)
            .unwrap_or(false)
    }

    #[inline(always)]
    pub fn exceeds_hard_limit(&self) -> bool {
        self.hard_limit_pages
            .map(|l| self.total_rss() > l)
            .unwrap_or(false)
    }
    ...
    pub fn limit_status(&self) -> RssLimitType {
        if self.exceeds_hard_limit() {
            RssLimitType::Hard
        } else if self.exceeds_soft_limit() {
            RssLimitType::Soft
        } else {
            RssLimitType::None
        }
    ...
    pub fn compute_oom_score(&mut self, total_mem_pages: u64) {
        ...
        let growth_bonus = if self.growth_rate_pages_per_sec > 1000.0 {
            100
        } else {
            0
        };
...
impl HolisticRssTracker {
    ...
    pub fn system_summary(&self) -> SystemRssSummary {
        ...
        SystemRssSummary {
            total_anon_pages: total_anon,
            total_file_pages: total_file,
            total_shmem_pages: total_shmem,
            total_swap_pages: total_swap,
            total_rss_pages: total_rss,
            total_memory_pages: self.total_memory_pages,
            rss_utilization: if self.total_memory_pages > 0 {
                total_rss as f64 / self.total_memory_pages as f64
            } else {
                0.0
            },
        }
    ...
    #[inline(always)]
    pub fn process(&self, pid: u32) -> Option<&ProcessRss> {
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.soft_limit_violators = self
            .processes
            .values()
            .filter(|p| p.exceeds_soft_limit())
            .count();
        self.stats.hard_limit_violators = self
            .processes
            .values()
            .filter(|p| p.exceeds_hard_limit())
            .count();
        if !self.processes.is_empty() {
            let n = self.processes.len() as f64;
            self.stats.avg_rss_pages = self.stats.total_rss_pages as f64 / n;
            self.stats.max_rss_pages = self
                .processes
                .values()
                .map(|p| p.total_rss())
                .max()
                .unwrap_or(0);
            self.stats.avg_growth_rate = self
                .processes
                .values()
                .map(|p| p.growth_rate_pages_per_sec)
                .sum::<f64>()
                / n;
            self.stats.top_oom_score = self
                .processes
                .values()
                .map(|p| p.oom_score)
                .max()
    ...
    #[inline(always)]
    pub fn stats(&self) -> &RssTrackerStats {
        &self.stats
    }
}
