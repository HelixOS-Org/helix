...
impl KernelProbe {
    ...
    pub fn enable(&mut self) {
        self.state = ProbeState::Registered;
    }
...
impl HolisticKprobes {
    ...
    pub fn register(&mut self, ptype: ProbeType, sym_hash: u64, addr: u64) -> u64 {
        ...
        self.next_id += 1;
        self.probes
            .insert(id, KernelProbe::new(id, ptype, sym_hash, addr));
        id
    }
    ...
    pub fn stats(&self) -> KprobesStats {
        let active = self
            .probes
            .values()
            .filter(|p| p.state == ProbeState::Active)
            .count() as u32;
