...
impl ProcessSecurityProfile {
    pub fn security_score(&self) -> f64 {
        ...
        if self.wx_compliant {
            score += 0.25;
        }
        if self.aslr_bits >= 28 {
            score += 0.20;
        } else if self.aslr_bits >= 20 {
            score += 0.10;
        }
        if self.guard_pages > 0 {
            score += 0.15;
        }
        if self.writable_exec_pages == 0 {
            score += 0.15;
        }
        if self.stack_canary {
            score += 0.10;
        }
        if self.relro_active {
            score += 0.10;
        }
        if self.pku_domain.is_some() {
            score += 0.05;
        }
        score
    }
...
impl MprotectHolisticManager {
    ...
    pub fn register_process(&mut self, profile: ProcessSecurityProfile) {
        if profile.wx_compliant {
            self.stats.wx_compliant_count += 1;
        }
    ...
    pub fn record_wx_violation(&mut self, pid: u64, page_addr: u64, now: u64) {
        ...
        if self.wx_timeline.len() > 1024 {
            self.wx_timeline.drain(..512);
        }
    ...
    fn recompute_scores(&mut self) {
        ...
        let total_score: f64 = self.profiles.values().map(|p| p.security_score()).sum();
    ...
    #[inline(always)]
    pub fn profile(&self, pid: u64) -> Option<&ProcessSecurityProfile> {
        self.profiles.get(&pid)
    }
    #[inline(always)]
    pub fn stats(&self) -> &MprotectHolisticStats {
        &self.stats
    }
}
