...
impl KprobeEntry {
    ...
    pub fn hit(&mut self, cpu: u32, pid: u64, now: u64) {
        self.hit_count += 1;
        if self.recent_hits.len() >= self.max_recent {
            self.recent_hits.remove(0);
        }
        self.recent_hits.push_back(ProbeHit {
            cpu,
            pid,
            timestamp: now,
            regs_hash: 0,
...
impl HolisticKprobeMgr {
    ...
    pub fn stats(&self) -> KprobeMgrStats {
        ...
        let kp = self
            .probes
            .values()
            .filter(|p| p.probe_type == ProbeType::Kprobe)
            .count() as u32;
        let kr = self
            .probes
            .values()
            .filter(|p| p.probe_type == ProbeType::Kretprobe)
            .count() as u32;
        let tp = self
            .probes
            .values()
            .filter(|p| p.probe_type == ProbeType::Tracepoint)
            .count() as u32;
        let overheads: Vec<u64> = self
            .probes
            .values()
            .filter(|p| p.overhead_ns > 0)
            .map(|p| p.overhead_ns)
            .collect();
        let avg = if overheads.is_empty() {
            0
        } else {
            overheads.iter().sum::<u64>() / overheads.len() as u64
        };
        KprobeMgrStats {
            total_probes: self.probes.len() as u32,
