...
impl HolisticMigratePages {
    ...
    pub fn register_node(&mut self, id: u32, tier: MemoryTier, capacity: u64) {
        self.nodes
            .insert(id, NodeMigrationState::new(id, tier, capacity));
    }

    #[inline(always)]
    pub fn track_page(&mut self, pfn: u64, node: u32, tier: MemoryTier, pid: u32, ts: u64) {
        self.pages
            .insert(pfn, TrackedPage::new(pfn, node, tier, pid, ts));
    ...
    pub fn suggest_migrations(&self) -> Vec<(u64, u32, MigrationReason)> {
        ...
        for page in self.pages.values() {
            match page.hotness {
                PageHotness::Hot
                    if page.current_tier != MemoryTier::FastDram
                        && page.current_tier != MemoryTier::LocalDram =>
                {
                    // Suggest promotion
                ...
                PageHotness::Frozen
                    if page.current_tier == MemoryTier::LocalDram
                        || page.current_tier == MemoryTier::FastDram =>
                {
                    // Suggest demotion
    ...
    fn find_fast_node(&self) -> Option<u32> {
        self.nodes
            .values()
            .filter(|n| {
                (n.tier == MemoryTier::FastDram || n.tier == MemoryTier::LocalDram)
                    && n.utilization() < 0.9
            })
            .min_by(|a, b| {
                a.utilization()
    ...
    fn find_slow_node(&self) -> Option<u32> {
        self.nodes
            .values()
            .filter(|n| {
                (n.tier == MemoryTier::Pmem || n.tier == MemoryTier::RemoteDram)
                    && n.utilization() < 0.9
            })
            .min_by(|a, b| {
                a.utilization()
    ...
    pub fn execute_migration(
        ...
        let page = match self.pages.get_mut(&pfn) {
            Some(p) => p,
            None => return false,
        };
        ...
        let to_tier = self
            ...
            .get(&to_node)
            .map(|n| n.tier)
            .unwrap_or(MemoryTier::LocalDram);
        ...
        if let Some(n) = self.nodes.get_mut(&from_node) {
            n.pages_migrated_out += 1;
            n.total_pages = n.total_pages.saturating_sub(1);
        }
        if let Some(n) = self.nodes.get_mut(&to_node) {
            n.pages_migrated_in += 1;
            n.total_pages += 1;
        }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.successful_migrations =
            self.requests.iter().filter(|r| r.success).count() as u64;
        self.stats.failed_migrations =
            self.stats.total_migrations - self.stats.successful_migrations;
        self.stats.promotions = self
            .requests
            .iter()
            .filter(|r| r.reason == MigrationReason::HotPromotion)
            .count() as u64;
        self.stats.demotions = self
            .requests
            .iter()
            .filter(|r| r.reason == MigrationReason::ColdDemotion)
            .count() as u64;
        self.stats.total_nodes = self.nodes.len();
        self.stats.hot_pages = self
            .pages
            .values()
            .filter(|p| p.hotness == PageHotness::Hot)
            .count();
        self.stats.cold_pages = self
            .pages
            .values()
            .filter(|p| matches!(p.hotness, PageHotness::Cold | PageHotness::Frozen))
            .count();
    }

    #[inline(always)]
    pub fn stats(&self) -> &MigratePageStats {
        &self.stats
    }
}
