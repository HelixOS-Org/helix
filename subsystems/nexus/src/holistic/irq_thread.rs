...
impl IrqThread {
    ...
    pub fn complete(&mut self) {
        self.state = IrqThreadState::Idle;
    }
...
impl HolisticIrqThread {
    ...
    pub fn handle(&mut self, irq: u32, latency_ns: u64) {
        if let Some(t) = self.threads.get_mut(&irq) {
            t.handle(latency_ns);
        }
    }
    ...
    pub fn stats(&self) -> IrqThreadStats {
        let active = self
            .threads
            .values()
            .filter(|t| t.state == IrqThreadState::Running)
            .count() as u32;
