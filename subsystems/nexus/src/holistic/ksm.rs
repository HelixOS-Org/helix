...
impl HolisticKsm {
    pub fn new(pages_to_scan: u32, sleep: u32) -> Self {
        Self {
            ...
            scan: KsmScanInfo::new(pages_to_scan, sleep),
        }
    }
    ...
    pub fn scan_page(&mut self, pfn: u64, checksum: u64) {
        ...
        if let Some(existing) = self.stable_tree.get_mut(&checksum) {
            ...
            self.stable_tree
                .insert(checksum, KsmPage::new(pfn, checksum));
    ...
    pub fn unmerge(&mut self, checksum: u64) {
        if let Some(p) = self.stable_tree.get_mut(&checksum) {
            if p.rmap_count > 1 {
                ...
                self.stable_tree.remove(&checksum);
                self.scan.pages_shared -= 1;
            }
        }
    }
    ...
    pub fn stats(&self) -> KsmStats {
        ...
        KsmStats {
            pages_shared: self.scan.pages_shared,
            pages_sharing: self.scan.pages_sharing,
            pages_unshared: self.scan.pages_unshared,
            pages_scanned: self.scan.pages_scanned,
            memory_saved_pages: saved,
        }
    }
}
