...
impl RcuCpuData {
    ...
    #[inline(always)]
    pub fn report_qs(&mut self, seq: u64) {
        self.qs_reported = true;
        self.gp_seq = seq;
    }
}
...
impl HolisticRcuSync {
    ...
    pub fn add_cpu(&mut self, cpu: u32) {
        self.cpu_data.insert(cpu, RcuCpuData::new(cpu));
    }
    ...
    pub fn start_gp(&mut self, now: u64) -> u64 {
        ...
        for data in self.cpu_data.values_mut() {
            data.qs_reported = false;
        }
        self.grace_periods
            .push(GracePeriod::new(self.current_gp_seq, now));
        self.current_gp_seq
    }
    ...
    pub fn report_qs(&mut self, cpu: u32) {
        ...
        if let Some(data) = self.cpu_data.get_mut(&cpu) {
            data.report_qs(seq);
        }
    }
    ...
    pub fn check_gp_complete(&mut self, now: u64) -> bool {
        let all_reported = self
            ...
            .all(|d| d.qs_reported);
        if all_reported {
            if let Some(gp) = self.grace_periods.last_mut() {
                if gp.state != GpState::Completed {
                    gp.complete(now);
                    return true;
                }
            }
    ...
    pub fn stats(&self) -> RcuSyncStats {
        let completed = self
            .grace_periods
            .iter()
            .filter(|gp| gp.state == GpState::Completed)
            .count() as u64;
        ...
        let durs: Vec<u64> = self
            .grace_periods
            .iter()
            .filter(|gp| gp.completed_at > 0)
            .map(|gp| gp.duration_ns())
            .collect();
