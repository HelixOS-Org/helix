...
impl HolisticAudit {
    ...
    pub fn analyze(&mut self, finding: &AuditHolisticFinding) {
        ...
        if finding.drop_rate() > 0.01 {
            self.stats.high_drop_rates += 1;
        }
        if finding.rules_active > 0 && finding.rules_effective == 0 {
            self.stats.ineffective_rules += 1;
        }
        if matches!(finding.metric, AuditHolisticMetric::LogIntegrity) {
            self.stats.integrity_issues += 1;
        }
    }
}
