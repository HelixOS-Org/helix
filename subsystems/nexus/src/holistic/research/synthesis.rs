...
impl HolisticSynthesis {
    ...
    pub fn coordination_plan(&mut self, tick: u64) -> CoordinationPlan {
        let mut ready: Vec<&PendingImprovement> = self
            .pending
            .values()
            .filter(|i| {
                i.status == SynthesisStatus::Pending || i.status == SynthesisStatus::Planned
            })
            .collect();
        ready.sort_by(|a, b| {
            b.expected_gain
        ...
        while !ready.is_empty() && stage_num < MAX_PLAN_STAGES {
            ...
            for imp in &ready {
                if assigned.contains(&imp.id) {
                    continue;
                }
                let conflicts_with_stage = imp.affected.iter().any(|s| stage_subs.contains(s));
                if !conflicts_with_stage && stage_ids.len() < ROLLOUT_BATCH {
                    stage_ids.push(imp.id);
                    for s in &imp.affected {
                        if !stage_subs.contains(s) {
                            stage_subs.push(*s);
                        }
                    }
            ...
            stages.push(PlanStage {
                ...
                improvement_ids: stage_ids,
                subsystems_touched: stage_subs,
                expected_gain: stage_gain,
                risk_score: risk,
            });
            ready = next_ready;
            stage_num += 1;
    ...
    pub fn staged_rollout(&mut self, plan: CoordinationPlan, tick: u64) -> SynthesisImpact {
        ...
        for stage in &plan.stages {
            for &imp_id in &stage.improvement_ids {
                if let Some(imp) = self.pending.get_mut(&imp_id) {
                    if imp.status == SynthesisStatus::Conflicted {
                        continue;
                    }
                    ...
                    if success {
                        ...
                        for s in &imp.affected {
                            if !subs_improved.contains(s) {
                                subs_improved.push(*s);
                            }
                        }
                        applied += 1;
        ...
        SynthesisImpact {
            ...
            net_gain,
            subsystems_improved: subs_improved,
            tick,
        }
    }
    ...
    pub fn system_improvement_score(&self) -> f32 {
        ...
        let integration = if applied > 1.0 {
            INTEGRATION_BONUS
        } else {
            1.0
        };
    ...
    fn cleanup_applied(&mut self) {
        let done: Vec<u64> = self
            ...
            .iter()
            .filter(|(_, i)| {
                i.status == SynthesisStatus::Applied || i.status == SynthesisStatus::Reverted
            })
            .map(|(&id, _)| id)
            .collect();
        for id in done {
