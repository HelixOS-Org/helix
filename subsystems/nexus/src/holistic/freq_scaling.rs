...
impl FreqDomain {
    pub fn new(id: u32, min: u64, max: u64) -> Self {
        Self {
            ...
            state: ScalingState::Active,
            transitions: 0,
            time_in_state: LinearMap::new(),
            last_transition: 0,
        }
    }
...
impl HolisticFreqScaling {
    ...
    pub fn stats(&self) -> FreqScalingStats {
        ...
        let boosted = self
            .domains
            .values()
            .filter(|d| d.state == ScalingState::Boosted)
            .count() as u32;
        let throttled = self
            .domains
            .values()
            .filter(|d| d.state == ScalingState::Throttled)
            .count() as u32;
        FreqScalingStats {
