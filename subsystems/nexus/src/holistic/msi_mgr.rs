...
impl MsiEntry {
    pub fn new(vector: u32, mt: MsiType, cpu: u32) -> Self {
        Self {
            ...
            msi_type: mt,
            target_cpu: cpu,
            addr: 0xFEE0_0000 | ((cpu as u64) << 12),
            data: vector as u32,
            masked: false,
            pending: false,
            irq_count: 0,
...
impl DeviceMsi {
    pub fn new(dev: u64, max: u32) -> Self {
        Self {
            device_id: dev,
            entries: Vec::new(),
            max_vectors: max,
    ...
    pub fn allocate(&mut self, vector: u32, mt: MsiType, cpu: u32) -> bool {
        if self.allocated_vectors >= self.max_vectors {
            return false;
        }
...
impl HolisticMsiMgr {
    ...
    pub fn register_device(&mut self, dev_id: u64, max_vectors: u32) {
        self.devices
            .insert(dev_id, DeviceMsi::new(dev_id, max_vectors));
    }
    ...
    pub fn stats(&self) -> MsiMgrStats {
        ...
        let msi: u32 = self
            ...
            .filter(|e| e.msi_type == MsiType::Msi)
            .count() as u32;
        let msix: u32 = self
            .devices
            .values()
            .flat_map(|d| d.entries.iter())
            .filter(|e| e.msi_type == MsiType::MsiX)
            .count() as u32;
