...
impl HolisticSignal {
    ...
    pub fn record(&mut self, rec: &SignalHolisticRecord) {
        ...
        if rec.pattern == SignalPattern::SignalStorm {
            self.stats.storms_detected += 1;
        }
        if rec.target_count > self.stats.peak_burst {
            self.stats.peak_burst = rec.target_count;
        }
        self.recent_latencies.push_back(rec.latency_ns);
        if self.recent_latencies.len() > 256 {
