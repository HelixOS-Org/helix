...
impl KWorkItem {
    pub fn new(id: u64, func_hash: u64, prio: KWorkPriority, flags: KWorkFlags, ts: u64) -> Self {
        Self {
            ...
            cpu_affinity: None,
            delay_ns: 0,
            status: KWorkStatus::Queued,
            retries: 0,
        }
    }

    #[inline(always)]
    pub fn delayed(mut self, delay: u64) -> Self {
        self.delay_ns = delay;
        self.status = KWorkStatus::Delayed;
        self
    }
    #[inline(always)]
    pub fn is_ready(&self, now: u64) -> bool {
        self.status == KWorkStatus::Queued
            || (self.status == KWorkStatus::Delayed
                && now.saturating_sub(self.queued_ts) >= self.delay_ns)
    }
    #[inline(always)]
    pub fn start(&mut self, worker: u64, ts: u64) {
        self.status = KWorkStatus::Running;
        self.worker_id = Some(worker);
        self.start_ts = ts;
    }
    #[inline(always)]
    pub fn complete(&mut self, ts: u64) {
        self.status = KWorkStatus::Complete;
        self.end_ts = ts;
    }
    #[inline(always)]
    pub fn fail(&mut self, ts: u64) {
        self.status = KWorkStatus::Failed;
        self.end_ts = ts;
        self.retries += 1;
    }
    #[inline(always)]
    pub fn cancel(&mut self) {
        self.status = KWorkStatus::Cancelled;
    }
    #[inline(always)]
    pub fn latency(&self) -> u64 {
        self.end_ts.saturating_sub(self.queued_ts)
    }
    #[inline(always)]
...
impl KWorker {
    ...
    #[inline(always)]
    pub fn assign(&mut self, work_id: u64, ts: u64) {
        self.state = KWorkerState::Running;
        self.current_work = Some(work_id);
        self.last_active_ts = ts;
    }
    #[inline(always)]
    pub fn finish(&mut self, ts: u64) {
        self.state = KWorkerState::Idle;
        self.current_work = None;
        self.tasks_completed += 1;
        self.busy_ns += ts.saturating_sub(self.last_active_ts);
    }
    #[inline(always)]
...
impl HolisticKthreadPool {
    pub fn new(stall_timeout: u64) -> Self {
        Self {
            ...
            stats: KThreadPoolStats::default(),
            next_worker_id: 1,
            next_work_id: 1,
            next_wq_id: 1,
    ...
    pub fn create_workqueue(&mut self, name_hash: u64, per_cpu: bool, max_workers: u32) -> u64 {
        ...
        self.next_wq_id += 1;
        self.workqueues
            .insert(id, KWorkqueue::new(id, name_hash, per_cpu, max_workers));
        id
    }
    ...
    pub fn queue_work(
        ...
        self.next_work_id += 1;
        self.work_items
            .insert(id, KWorkItem::new(id, func_hash, prio, flags, ts));
        id
    }
    ...
    pub fn schedule(&mut self, now: u64) -> Vec<(u64, u64)> {
        ...
        let idle: Vec<u64> = self
            .workers
            .values()
            .filter(|w| w.state == KWorkerState::Idle)
            .map(|w| w.id)
            .collect();
        ...
        ready.sort_by(|a, b| {
            let pa = self
                ...
                .get(a)
                .map(|w| w.priority)
                .unwrap_or(KWorkPriority::Normal);
            let pb = self
                .work_items
                .get(b)
                .map(|w| w.priority)
        ...
        for (wi, wk) in ready.iter().zip(idle.iter()) {
            if let Some(work) = self.work_items.get_mut(wi) {
                work.start(*wk, now);
            }
            if let Some(worker) = self.workers.get_mut(wk) {
                worker.assign(*wi, now);
            }
    ...
    pub fn complete_work(&mut self, work_id: u64, ts: u64) {
        if let Some(w) = self.work_items.get_mut(&work_id) {
            let wk = w.worker_id;
            w.complete(ts);
            if let Some(wk_id) = wk {
                if let Some(worker) = self.workers.get_mut(&wk_id) {
                    worker.finish(ts);
                }
            }
        }
    ...
    pub fn detect_stalls(&self, now: u64) -> Vec<u64> {
        self.workers
            .values()
            .filter(|w| {
                w.state == KWorkerState::Running
                    && now.saturating_sub(w.last_active_ts) > self.stall_timeout_ns
            })
            .map(|w| w.id)
            .collect()
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.idle_workers = self
            .workers
            .values()
            .filter(|w| w.state == KWorkerState::Idle)
            .count();
        self.stats.busy_workers = self
            .workers
            .values()
            .filter(|w| w.state == KWorkerState::Running)
            .count();
        self.stats.pending_work = self
            .work_items
            .values()
            .filter(|w| matches!(w.status, KWorkStatus::Queued | KWorkStatus::Delayed))
            .count();
        self.stats.total_completed = self
            .work_items
            .values()
            .filter(|w| w.status == KWorkStatus::Complete)
            .count() as u64;
        let done: Vec<&KWorkItem> = self
            .work_items
            .values()
            .filter(|w| w.status == KWorkStatus::Complete)
            .collect();
        if !done.is_empty() {
            self.stats.avg_latency_ns =
                done.iter().map(|w| w.latency()).sum::<u64>() / done.len() as u64;
            self.stats.avg_exec_ns =
                done.iter().map(|w| w.exec_time()).sum::<u64>() / done.len() as u64;
        }
        self.stats.workqueues = self.workqueues.len();
    }

    #[inline(always)]
    pub fn worker(&self, id: u64) -> Option<&KWorker> {
        self.workers.get(&id)
    }
    #[inline(always)]
    pub fn work(&self, id: u64) -> Option<&KWorkItem> {
        self.work_items.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &KThreadPoolStats {
        &self.stats
    }
}
