...
impl AppEventfd {
    ...
    pub fn create(&mut self, initval: u64, flags: EventfdFlags, pid: u64, now: u64) -> u64 {
        ...
        self.next_id += 1;
        self.instances
            .insert(id, EventfdInstance::new(id, initval, flags, pid, now));
        id
    }
    ...
    pub fn write(&mut self, id: u64, val: u64) -> bool {
        self.instances.get_mut(&id).map_or(false, |e| e.write(val))
    }
    #[inline(always)]
    pub fn read(&mut self, id: u64) -> Option<u64> {
        Some(self.instances.get_mut(&id)?.read())
    }
    ...
    pub fn read(&mut self) -> u64 {
        ...
        if self.semaphore {
            if self.counter > 0 {
                ...
                self.state = EventfdV2State::Blocked;
                0
            }
        } else {
            let v = self.counter;
...
impl AppEventfdV2 {
    ...
    pub fn create(&mut self, initial: u64, semaphore: bool, now: u64) -> u64 {
        ...
        self.next_id += 1;
        self.instances
            .insert(id, EventfdV2Instance::new(id, initial, semaphore, now));
        id
    }
    ...
    pub fn close(&mut self, id: u64) {
        if let Some(inst) = self.instances.get_mut(&id) {
            inst.state = EventfdV2State::Closed;
        }
    }
    ...
    pub fn stats(&self) -> EventfdV2AppStats {
        ...
        let signaled = self
            .instances
            .values()
            .filter(|i| i.state == EventfdV2State::Signaled)
            .count() as u32;
        EventfdV2AppStats {
            total_instances: self.instances.len() as u32,
            total_writes: writes,
...
impl EventfdV3AppRecord {
    pub fn new(op: EventfdV3AppOp) -> Self {
        Self {
            op,
            flags: 0,
            value: 0,
...
/// Main app eventfd v3
#[derive(Debug)]
pub struct AppEventfdV3 {
    pub stats: EventfdV3AppStats,
}

impl AppEventfdV3 {
    pub fn new() -> Self {
        Self {
            stats: EventfdV3AppStats {
                total_ops: 0,
                created: 0,
                reads: 0,
                writes: 0,
