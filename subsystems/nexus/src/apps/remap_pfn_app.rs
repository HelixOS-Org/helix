...
impl AppRemapPfn {
    ...
    pub fn remap(
        ...
        pfn: u64,
        pages: u64,
        remap_type: RemapPfnType,
        pid: u64,
    ) -> u64 {
        let id = self.next_id;
        self.next_id += 1;
        self.mappings.insert(id, PfnMapping {
            id,
            virt_addr,
            pfn_start: pfn,
            page_count: pages,
    ...
    pub fn stats(&self) -> RemapPfnAppStats {
        ...
        let dev = self
            .mappings
            .values()
            .filter(|m| m.remap_type == RemapPfnType::DeviceMemory)
            .count() as u32;
        let fb = self
            .mappings
            .values()
            .filter(|m| m.remap_type == RemapPfnType::Framebuffer)
            .count() as u32;
        RemapPfnAppStats {
            total_mappings: self.mappings.len() as u32,
            total_pages: pages,
            device_mappings: dev,
            framebuffer_mappings: fb,
            total_bytes_mapped: pages * 4096,
        }
    }
}
