...
impl AppBpf {
    ...
    pub fn load_prog(&mut self, ptype: BpfProgType, insns: u32) -> u64 {
        let id = self.next_id;
        self.next_id += 1;
        self.progs.insert(id, BpfProgram {
            id,
            prog_type: ptype,
            insn_count: insns,
            verified: true,
    ...
    #[inline(always)]
    pub fn attach(&mut self, prog_id: u64) {
        if let Some(p) = self.progs.get_mut(&prog_id) {
            p.attached = true;
        }
    }
    #[inline(always)]
    pub fn detach(&mut self, prog_id: u64) {
        if let Some(p) = self.progs.get_mut(&prog_id) {
            p.attached = false;
        }
    }
    ...
    pub fn stats(&self) -> BpfAppStats {
        ...
        BpfAppStats {
            total_progs: self.progs.len() as u32,
            total_maps: self.maps.len() as u32,
            attached_progs: attached,
            total_runs: runs,
        }
    }
}
