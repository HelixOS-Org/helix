...
impl MincoreQuery {
    ...
    pub fn resident_count(&self) -> usize {
        self.results
            .iter()
            .filter(|&&r| r == PageResidency::Resident)
            .count()
    }
    #[inline(always)]
    pub fn resident_ratio(&self) -> f64 {
        if self.results.is_empty() {
            ...
            self.resident_count() as f64 / self.results.len() as f64
        }
    }
}
...
impl ProcessResidencyInfo {
    ...
    pub fn overall_residency(&self) -> f64 {
        if self.total_pages_queried == 0 {
            0.0
        } else {
            self.total_resident as f64 / self.total_pages_queried as f64
        }
    }
}
...
impl AppMincore {
    ...
    pub fn register(&mut self, pid: u64) {
        self.processes.insert(pid, ProcessResidencyInfo::new(pid));
    }
    #[inline(always)]
    ...
    pub fn query(&mut self, query: &MincoreQuery) {
        if let Some(p) = self.processes.get_mut(&query.pid) {
    ...
    pub fn stats(&self) -> MincoreAppStats {
        ...
        let residencies: Vec<f64> = self
            .processes
            .values()
            .map(|p| p.overall_residency())
            .collect();
        let avg = if residencies.is_empty() {
            0.0
        } else {
            residencies.iter().sum::<f64>() / residencies.len() as f64
        };
        MincoreAppStats {
            tracked_processes: self.processes.len() as u32,
            total_queries: queries,
            total_pages_queried: pages,
            avg_residency: avg,
        }
    }
}
...
impl ProcessResidencyV2 {
    ...
    pub fn query(&mut self, addr: u64, len: u64, pages: u32, resident: u32, now: u64) {
        self.queries.push(MincoreV2Query {
            addr,
            length: len,
            page_count: pages,
    ...
    pub fn snapshot(&mut self, total: u64, resident: u64, swapped: u64, file: u64, now: u64) {
        ...
        self.snapshots.push(ResidencySnapshotV2 {
            pid: self.pid,
            total_pages: total,
...
impl AppMincoreV2 {
    ...
    pub fn stats(&self) -> MincoreV2AppStats {
        ...
        let avg = if self.procs.is_empty() {
            0.0
        } else {
            self.procs.values().map(|p| p.avg_residency()).sum::<f64>() / self.procs.len() as f64
        };
        MincoreV2AppStats {
            tracked_procs: self.procs.len() as u32,
            total_queries: queries,
            total_pages_checked: pages,
            avg_residency: avg,
        }
    }
}
