...
impl CgroupNode {
    pub fn new(id: u64, parent: u64, depth: u8) -> Self {
        Self {
            ...
            freeze: CgroupFreezeState::Thawed,
            children: Vec::new(),
            processes: Vec::new(),
            cpu_usage_us: 0,
            mem_current: 0,
    ...
    pub fn mem_utilization(&self) -> f64 {
        if self.limits.mem_max == u64::MAX {
            0.0
        } else {
            self.mem_current as f64 / self.limits.mem_max as f64
        }
    }
}
...
impl AppCgroup {
    ...
    pub fn stats(&self) -> CgroupAppStats {
        ...
        let frozen = self
            .groups
            .values()
            .filter(|g| g.freeze == CgroupFreezeState::Frozen)
            .count() as u32;
