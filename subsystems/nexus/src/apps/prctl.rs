...
impl ProcessPrctlState {
    pub fn new(pid: u64) -> Self {
        Self {
            pid,
            name: String::new(),
            dumpable: DumpableSetting::Dumpable,
...
impl AppPrctlV2 {
    ...
    pub fn prctl(&mut self, pid: u64, opt: PrctlOption, arg: u64, now: u64) -> i64 {
        ...
        if self.audit_log.len() >= self.max_audit {
            self.audit_log.drain(..self.max_audit / 4);
        }
        self.audit_log.push(PrctlAuditRecord {
            pid,
            option: opt,
            arg,
            result,
    ...
    pub fn stats(&self) -> PrctlV2Stats {
        ...
        let strict = self
            .processes
            .values()
            .filter(|p| p.seccomp == SeccompMode::Strict)
            .count() as u32;
        let filter = self
            .processes
            .values()
            .filter(|p| p.seccomp == SeccompMode::Filter)
            .count() as u32;
        let sr = self
            .processes
            .values()
            .filter(|p| p.child_subreaper)
            .count() as u32;
        PrctlV2Stats {
            total_processes: self.processes.len() as u32,
            total_calls: calls,
            no_new_privs_count: nnp,
            seccomp_strict_count: strict,
            seccomp_filter_count: filter,
            subreaper_count: sr,
        }
    }
}
