...
impl EpollEventMask {
    ...
    pub fn is_read(&self) -> bool {
        self.has(Self::EPOLLIN)
    }
    #[inline(always)]
    pub fn is_write(&self) -> bool {
        self.has(Self::EPOLLOUT)
    }
    #[inline(always)]
    pub fn is_edge(&self) -> bool {
        self.has(Self::EPOLLET)
    }
}
...
impl EpollRegisteredFd {
    pub fn new(fd: i32, events: EpollEventMask, ts: u64) -> Self {
        let trigger = if events.is_edge() {
            EpollTriggerMode::EdgeTriggered
        } else if events.has(EpollEventMask::EPOLLONESHOT) {
            EpollTriggerMode::OneShot
        } else {
            EpollTriggerMode::LevelTriggered
        };
        Self {
            fd,
            events,
            ...
            last_ready_ts: 0,
            registered_ts: ts,
            stale_threshold_ns: 30_000_000_000,
        }
    }
    ...
    pub fn is_stale(&self, now: u64) -> bool {
        if self.last_ready_ts == 0 {
            return now - self.registered_ts > self.stale_threshold_ns;
        }
...
impl EpollInstance {
    ...
    pub fn record_wait(&mut self, events_returned: u32, latency_ns: u64, ts: u64) {
        ...
        if events_returned > self.max_events_per_wait {
            self.max_events_per_wait = events_returned;
        }
...
impl ThunderingHerdDetector {
    pub fn new(threshold: u32) -> Self {
        Self {
            recent_concurrent_wakes: Vec::new(),
            max_history: 64,
            threshold,
            detected_count: 0,
        }
    ...
    pub fn record_wake(&mut self, ts: u64, woken_count: u32) {
        ...
        if self.recent_concurrent_wakes.len() > self.max_history {
            self.recent_concurrent_wakes.remove(0);
        }
        if woken_count >= self.threshold {
    ...
    pub fn is_thundering(&self) -> bool {
        if self.recent_concurrent_wakes.len() < 3 {
            return false;
        }
...
impl AppsEpollMgr {
    ...
    pub fn create(&mut self, pid: u64, epfd: i32, ts: u64) {
        self.instances
            .insert((pid, epfd), EpollInstance::new(epfd, pid, ts));
    }

    #[inline(always)]
    pub fn ctl_add(&mut self, pid: u64, epfd: i32, fd: i32, events: EpollEventMask, ts: u64) {
        if let Some(inst) = self.instances.get_mut(&(pid, epfd)) {
            inst.add_fd(fd, events, ts);
        }
    }
    ...
    pub fn record_wait(&mut self, pid: u64, epfd: i32, events: u32, latency: u64, ts: u64) {
        if let Some(inst) = self.instances.get_mut(&(pid, epfd)) {
            inst.record_wait(events, latency, ts);
        }
    }

    #[inline(always)]
    pub fn destroy(&mut self, pid: u64, epfd: i32) {
        self.instances.remove(&(pid, epfd));
    }
    ...
    pub fn recompute(&mut self, now: u64) {
        ...
        self.stats.stale_fd_count = self
            .instances
            .values()
            .map(|i| i.stale_fds(now).len())
            .sum();
        self.stats.thundering_herd_events = self.herd_detector.detected_count;
        if self.stats.total_instances > 0 {
            self.stats.avg_fds_per_instance =
                self.stats.total_monitored_fds as f64 / self.stats.total_instances as f64;
        }
    }

    #[inline(always)]
    pub fn instance(&self, pid: u64, epfd: i32) -> Option<&EpollInstance> {
        self.instances.get(&(pid, epfd))
    }
    #[inline(always)]
    pub fn stats(&self) -> &EpollMgrStats {
        &self.stats
    }
}
