...
impl AppFadvise {
    ...
    pub fn track(&mut self, fd: i32, pid: u64, inode: u64) {
        ...
        self.trackers
            .insert(key, FileAccessTracker::new(fd, pid, inode));
    }
    ...
    pub fn stats(&self) -> FadviseAppStats {
        ...
        let seq = self
            .trackers
            .values()
            .filter(|t| t.detect_pattern() == FadviseAdvice::Sequential)
            .count() as u32;
        let rand = self
            .trackers
            .values()
            .filter(|t| t.detect_pattern() == FadviseAdvice::Random)
            .count() as u32;
        let bytes: u64 = self.trackers.values().map(|t| t.read_bytes).sum();
        FadviseAppStats {
            tracked_files: self.trackers.len() as u32,
            total_advices: advices,
            sequential_files: seq,
            random_files: rand,
...
impl FileAdviceV2 {
    ...
    pub fn advise(&mut self, offset: u64, len: u64, advice: FadviseV2Advice, now: u64) {
        self.regions.push(FadviseV2Region {
            offset,
            length: len,
            advice,
            applied_at: now,
...
impl AppFadviseV2 {
    ...
    pub fn stats(&self) -> FadviseV2AppStats {
        ...
        let rates: f64 = if self.files.is_empty() {
            0.0
        } else {
            self.files.values().map(|f| f.hit_rate()).sum::<f64>() / self.files.len() as f64
        };
        FadviseV2AppStats {
            tracked_files: self.files.len() as u32,
            total_advise_calls: calls,
            total_bytes_advised: bytes,
            avg_hit_rate: rates,
        }
    }
}
...
impl AppFadviseV3 {
    ...
    pub fn record_fadvise(
        ...
        let state = self
            .files
            .entry(inode)
            .or_insert_with(|| FileReadaheadState::new(inode));
        match advice {
...
impl FadviseV4Record {
    pub fn new(fd: i32, advice: FadviseV4Advice) -> Self {
        Self {
            fd,
            advice,
            result: FadviseV4Result::Success,
            offset: 0,
            length: 0,
        }
    }
    ...
    pub fn is_readahead_hint(&self) -> bool {
        matches!(
            self.advice,
            FadviseV4Advice::Sequential | FadviseV4Advice::WillNeed
        )
    }
    ...
    pub fn is_drop_hint(&self) -> bool {
        matches!(
            self.advice,
            FadviseV4Advice::DontNeed | FadviseV4Advice::NoReuse
        )
    }
}
...
impl AppFadviseV4 {
    ...
    pub fn record(&mut self, rec: &FadviseV4Record) {
        self.stats.total_calls += 1;
        if rec.result != FadviseV4Result::Success {
            self.stats.errors += 1;
            return;
        }
        match rec.advice {
            FadviseV4Advice::Sequential => {
                ...
                self.windows.insert(
                    rec.fd,
                    ReadaheadWindow::new(rec.fd, rec.offset, rec.length.max(131072)),
                );
            },
