...
impl PipeInstance {
    pub fn new(
        ...
        Self {
            ...
            reader_pid: reader,
            writer_pid: writer,
            reader_state: EndpointState::Open,
            writer_state: EndpointState::Open,
...
impl AppsPipeMgr {
    ...
    pub fn create(
        ...
        self.pipes.insert(
            id,
            PipeInstance::new(id, kind, rfd, wfd, reader, writer, buf_size, ts),
        );
        id
    }
    ...
    pub fn record_write(&mut self, id: u64, bytes: usize, blocked: bool, ts: u64) {
        if let Some(p) = self.pipes.get_mut(&id) {
            p.record_write(bytes, blocked, ts);
        }
    }
    ...
    pub fn record_read(&mut self, id: u64, bytes: usize, blocked: bool, ts: u64) {
        if let Some(p) = self.pipes.get_mut(&id) {
            p.record_read(bytes, blocked, ts);
        }
    }
    ...
    pub fn gc_closed(&mut self) {
        self.pipes.retain(|_, p| !p.is_closed());
    }
    ...
    #[inline(always)]
    pub fn stats(&self) -> &PipeMgrStats {
        &self.stats
    }
}
