...
impl PtyPair {
    ...
    #[inline(always)]
    pub fn set_foreground(&mut self, pgrp: u64) {
        self.foreground_pgrp = pgrp;
    }
    #[inline(always)]
    pub fn set_winsize(&mut self, ws: PtyWinSize) {
        self.winsize = ws;
    }
    ...
    pub fn is_idle(&self, now: u64, threshold_ns: u64) -> bool {
        if self.last_io_ts == 0 {
            return now - self.created_ts > threshold_ns;
        }
...
impl JobControlState {
    ...
    pub fn remove_pid(&mut self, pid: u64) {
        for group in self.process_groups.values_mut() {
    ...
    pub fn mark_stopped(&mut self, pgrp: u64) {
        if !self.stopped_groups.contains(&pgrp) {
            self.stopped_groups.push(pgrp);
        }
    }

    #[inline(always)]
    pub fn mark_continued(&mut self, pgrp: u64) {
        self.stopped_groups.retain(|&g| g != pgrp);
    }

    pub fn detect_orphaned(&mut self) -> Vec<u64> {
        // simplified: groups with no parent in session
        let all_pids: Vec<u64> = self
            .process_groups
            .values()
...
impl AppsPtyMgr {
    pub fn new() -> Self {
        Self {
            ...
            next_id: 1,
            idle_threshold_ns: 60_000_000_000,
    ...
    pub fn allocate(&mut self, master: i32, slave: i32, name: String, ts: u64) -> u64 {
        ...
        self.next_id += 1;
        self.ptys
            .insert(id, PtyPair::new(id, master, slave, name, ts));
        id
    }
    ...
    pub fn activate(&mut self, pty_id: u64, session_leader: u64) {
        if let Some(pty) = self.ptys.get_mut(&pty_id) {
            ...
            self.sessions
                .entry(session_leader)
                .or_insert_with(|| JobControlState::new(session_leader));
        }
    }
    ...
    #[inline(always)]
    pub fn close(&mut self, pty_id: u64) {
    ...
    pub fn recompute(&mut self, now: u64) {
        ...
        self.stats.active_ptys = self
            .ptys
            .values()
            .filter(|p| p.state == PtyState::Active)
            .count();
        self.stats.hung_ptys = self
            .ptys
            .values()
            .filter(|p| p.state == PtyState::Hung)
            .count();
        ...
        self.stats.idle_ptys = self
            .ptys
            .values()
            .filter(|p| p.is_idle(now, self.idle_threshold_ns))
            .count();
    }

    #[inline(always)]
    pub fn pty(&self, id: u64) -> Option<&PtyPair> {
        self.ptys.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &PtyMgrStats {
        &self.stats
    }
}
