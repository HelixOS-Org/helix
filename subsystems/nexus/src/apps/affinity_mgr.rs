...
impl CpuSet {
    ...
    pub fn set(&mut self, cpu: u32) {
        if cpu < 256 {
            self.mask[(cpu / 64) as usize] |= 1u64 << (cpu % 64);
        }
    }
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn has(&self, cpu: u32) -> bool {
        if cpu < 256 {
            self.mask[(cpu / 64) as usize] & (1u64 << (cpu % 64)) != 0
        } else {
            false
        }
    }
    ...
    pub fn intersect(&self, other: &Self) -> Self {
        let mut r = Self::new();
        for i in 0..4 {
            r.mask[i] = self.mask[i] & other.mask[i];
        }
        r
    }
...
impl ThreadAffinity {
    pub fn new(tid: u64) -> Self {
        Self {
            ...
            cpu_set: CpuSet::all(256),
            effective: CpuSet::all(256),
            migrations: 0,
            last_cpu: 0,
            pin_count: 0,
        }
    ...
    #[inline(always)]
    pub fn bind(&mut self, set: CpuSet) {
        self.cpu_set = set.clone();
        self.effective = set;
    }
    #[inline(always)]
    pub fn migrate(&mut self, new_cpu: u32) {
        if new_cpu != self.last_cpu {
            self.migrations += 1;
            self.last_cpu = new_cpu;
        }
    }
    #[inline(always)]
    pub fn pin(&mut self, cpu: u32) {
        let mut s = CpuSet::new();
        s.set(cpu);
        self.cpu_set = s.clone();
        self.effective = s;
        self.pin_count += 1;
    }
}
...
impl AppAffinityMgr {
    ...
    pub fn register(&mut self, tid: u64) {
        self.threads.insert(tid, ThreadAffinity::new(tid));
    }
    #[inline(always)]
    pub fn set_affinity(&mut self, tid: u64, set: CpuSet) {
