...
impl ProcessIoProfile {
    pub fn new(pid: u64) -> Self {
        Self {
            ...
            read_ios: 0,
            write_ios: 0,
            pattern: IoPattern::Mixed,
            size_dist: SizeBucket::default(),
            latency: LatencyTracker::new(1024),
            last_offset: 0,
            sequential_count: 0,
            random_count: 0,
            prio: IoPrioClass::BestEffort,
            merges: 0,
        }
    }
    ...
    pub fn record(&mut self, rec: &IoRecord) {
        match rec.direction {
            IoDirection::Read => {
                self.read_ios += 1;
                self.read_bytes += rec.size as u64;
            },
            IoDirection::Write => {
                self.write_ios += 1;
                self.write_bytes += rec.size as u64;
            },
            _ => {},
        }
        ...
        let diff = if rec.offset >= self.last_offset {
            rec.offset - self.last_offset
        } else {
            self.last_offset - rec.offset
        };
        if diff <= rec.size as u64 * 2 {
    ...
    pub fn rw_ratio(&self) -> f64 {
        if self.write_ios == 0 {
            ...
            self.read_ios as f64 / self.write_ios as f64
        }
    }
    #[inline(always)]
    ...
    pub fn avg_size(&self) -> u64 {
        let t = self.total_ios();
        if t == 0 { 0 } else { self.total_bytes() / t }
    }
    #[inline(always)]
    pub fn merge_pct(&self) -> f64 {
        let t = self.total_ios();
        if t == 0 {
            0.0
        } else {
            self.merges as f64 / t as f64 * 100.0
        }
    }
}
...
impl AppsIoProfiler {
    ...
    pub fn track(&mut self, pid: u64) {
        self.profiles
            .entry(pid)
            .or_insert_with(|| ProcessIoProfile::new(pid));
    }
    #[inline(always)]
    ...
    pub fn recompute(&mut self) {
        ...
        if !self.profiles.is_empty() {
            self.stats.avg_latency_ns =
                self.profiles.values().map(|p| p.latency.avg()).sum::<u64>()
                    / self.profiles.len() as u64;
            let total_seq: u64 = self.profiles.values().map(|p| p.sequential_count).sum();
            let total_ops: u64 = self
                .profiles
                .values()
                .map(|p| p.sequential_count + p.random_count)
                .sum();
            self.stats.sequential_pct = if total_ops == 0 {
                0.0
            } else {
                total_seq as f64 / total_ops as f64 * 100.0
            };
        }
    }

    #[inline(always)]
    pub fn profile(&self, pid: u64) -> Option<&ProcessIoProfile> {
        self.profiles.get(&pid)
    }
    #[inline(always)]
    pub fn stats(&self) -> &IoProfilerStats {
        &self.stats
    }
}
