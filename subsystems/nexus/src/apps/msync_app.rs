...
impl MappingSyncTracker {
    ...
    #[inline(always)]
    pub fn mark_dirty(&mut self, pages: u32) {
        self.dirty_pages += pages;
    }
}
...
impl AppMsync {
    ...
    #[inline(always)]
    pub fn track_mapping(&mut self, addr: u64, len: u64) {
        self.mappings
            .insert(addr, MappingSyncTracker::new(addr, len));
    }

    #[inline(always)]
    pub fn sync(&mut self, addr: u64, pages: u32, flag: MsyncFlag, ts: u64) {
        match flag {
            MsyncFlag::Sync => self.sync_ops += 1,
            MsyncFlag::Async => self.async_ops += 1,
            _ => {},
        }
        if let Some(m) = self.mappings.get_mut(&addr) {
            m.record_sync(pages, ts);
        }
    }
    ...
    #[inline(always)]
    pub fn untrack(&mut self, addr: u64) {
    ...
    pub fn stats(&self) -> MsyncAppStats {
        ...
        MsyncAppStats {
            tracked_mappings: self.mappings.len() as u32,
            total_syncs: syncs,
            total_pages_synced: pages,
