...
impl ProcessLimits {
    pub fn new(pid: u64) -> Self {
        ...
        for i in 0..16u8 {
            limits.insert(i, Rlimit::unlimited());
        }
        Self {
            pid,
            limits,
...
impl AppPrlimit {
    ...
    pub fn register(&mut self, pid: u64) {
        self.processes.insert(pid, ProcessLimits::new(pid));
    }
    ...
    pub fn set_limit(&mut self, pid: u64, resource: RlimitResource, limit: Rlimit) -> bool {
        self.processes
            .get_mut(&pid)
            .map_or(false, |p| p.set(resource, limit))
    }
    ...
    pub fn stats(&self) -> PrlimitAppStats {
        ...
        let unlimited: u32 = self
            .processes
            .values()
            .map(|p| p.limits.values().filter(|l| l.is_unlimited()).count() as u32)
            .sum();
        PrlimitAppStats {
