...
impl MerkleNode {
    ...
    pub fn branch(left: &MerkleNode, right: &MerkleNode) -> Self {
        ...
        Self {
            hash: h,
            level: core::cmp::max(left.level, right.level) + 1,
...
impl RepairTask {
    ...
    pub fn start(&mut self) {
        self.state = RepairTaskState::Running;
    }
    #[inline(always)]
    pub fn complete(&mut self, keys: u32, bytes: u64, ts: u64) {
        self.state = RepairTaskState::Complete;
        self.keys_repaired = keys;
        self.bytes_transferred = bytes;
        self.end_ts = ts;
    }
    #[inline(always)]
    pub fn fail(&mut self, ts: u64) {
        self.state = RepairTaskState::Failed;
        self.end_ts = ts;
        self.retries += 1;
    }
    #[inline(always)]
    pub fn cancel(&mut self) {
        self.state = RepairTaskState::Cancelled;
    }
    #[inline(always)]
...
impl CoopAntiEntropyV2 {
    pub fn new(max_concurrent: u32, sync_interval: u64) -> Self {
        Self {
            local_tree: BTreeMap::new(),
            peers: BTreeMap::new(),
            tasks: BTreeMap::new(),
            queue: Vec::new(),
            convergence_history: Vec::new(),
            stats: AntiEntropyV2Stats::default(),
            next_task_id: 1,
            max_concurrent,
            sync_interval_ns: sync_interval,
        }
    }
    ...
    pub fn add_peer(&mut self, peer: u64) {
        self.peers
            .entry(peer)
            .or_insert_with(|| PeerEntropyState::new(peer));
    }
    ...
    pub fn find_divergences(
        &self,
        _peer_id: u64,
        remote_keys: &BTreeMap<u64, u64>,
    ) -> Vec<Divergence> {
        let mut divs = Vec::new();
        // Keys in local but not remote or different
        for (&k, node) in &self.local_tree {
            match remote_keys.get(&k) {
                None => divs.push(Divergence {
                    range_start: k,
                    range_end: k,
                    local_hash: node.hash,
                    remote_hash: 0,
                    key_count: 1,
                    severity: 1.0,
                }),
                Some(&rh) if rh != node.hash => divs.push(Divergence {
                    range_start: k,
                    range_end: k,
                    local_hash: node.hash,
                    remote_hash: rh,
    ...
    pub fn run_next(&mut self) -> Option<u64> {
        let running = self
            .tasks
            .values()
            .filter(|t| t.state == RepairTaskState::Running)
            .count() as u32;
        ...
        let tid = self
            ...
            .find(|&&id| {
                self.tasks
                    .get(&id)
                    .map(|t| t.state == RepairTaskState::Queued)
                    .unwrap_or(false)
            })
            .copied()?;
    ...
    pub fn record_convergence(&mut self, ts: u64) {
        ...
        let entropy = if ratio >= 1.0 {
            0.0
        } else {
            -(ratio * libm::log(ratio) + (1.0 - ratio) * libm::log(1.0 - ratio))
        };
        self.convergence_history.push(ConvergenceMetric {
            ts,
    ...
    #[inline(always)]
    pub fn peer(&self, id: u64) -> Option<&PeerEntropyState> {
        self.peers.get(&id)
    }
    #[inline(always)]
    pub fn task(&self, id: u64) -> Option<&RepairTask> {
        self.tasks.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &AntiEntropyV2Stats {
        &self.stats
    }
    #[inline(always)]
    pub fn convergence_history(&self) -> &[ConvergenceMetric] {
