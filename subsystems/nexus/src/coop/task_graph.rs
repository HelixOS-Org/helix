...
impl GraphTask {
    ...
    #[inline(always)]
    pub fn add_dep(&mut self, dep: u64) {
        if !self.deps.contains(&dep) {
            self.deps.push(dep);
        }
    }
    #[inline(always)]
    pub fn add_dependent(&mut self, d: u64) {
        if !self.dependents.contains(&d) {
            self.dependents.push(d);
        }
    }

    #[inline(always)]
    pub fn start(&mut self, worker: u64, ts: u64) {
        self.status = GraphTaskStatus::Running;
        self.assigned_to = Some(worker);
        self.start_ts = ts;
    }
    #[inline(always)]
    pub fn complete(&mut self, result: u64, ts: u64) {
        self.status = GraphTaskStatus::Complete;
        self.result_hash = result;
        self.end_ts = ts;
        self.cost_ns = ts.saturating_sub(self.start_ts);
    }
    #[inline(always)]
    pub fn fail(&mut self, ts: u64) {
        self.retries += 1;
        if self.retries >= self.max_retries {
            self.status = GraphTaskStatus::Failed;
        } else {
            self.status = GraphTaskStatus::Pending;
        }
        self.end_ts = ts;
    }
    #[inline(always)]
    pub fn cancel(&mut self) {
        self.status = GraphTaskStatus::Cancelled;
    }
    #[inline(always)]
    pub fn skip(&mut self) {
        self.status = GraphTaskStatus::Skipped;
    }
    #[inline(always)]
    pub fn use_cache(&mut self, result: u64) {
        self.status = GraphTaskStatus::Cached;
        self.result_hash = result;
        self.cached = true;
    }
    ...
    pub fn is_terminal(&self) -> bool {
        matches!(
            self.status,
            GraphTaskStatus::Complete
                | GraphTaskStatus::Failed
                | GraphTaskStatus::Cancelled
                | GraphTaskStatus::Skipped
                | GraphTaskStatus::Cached
        )
    }
    #[inline(always)]
    pub fn is_success(&self) -> bool {
        matches!(
            self.status,
            GraphTaskStatus::Complete | GraphTaskStatus::Cached
        )
    }
}
...
impl CoopTaskGraph {
    ...
    pub fn add_task(
        &mut self,
        name_hash: u64,
        prio: GraphTaskPriority,
        policy: FailurePolicy,
    ) -> u64 {
        let id = self.next_id;
        self.next_id += 1;
        self.tasks
            .insert(id, GraphTask::new(id, name_hash, prio, policy));
        id
    }
    ...
    pub fn topological_sort(&mut self) -> bool {
        ...
        queue.sort_by(|a, b| {
            let pa = self
                ...
                .get(a)
                .map(|t| t.priority)
                .unwrap_or(GraphTaskPriority::Normal);
            let pb = self
                .tasks
                .get(b)
                .map(|t| t.priority)
    ...
    pub fn ready_tasks(&self) -> Vec<u64> {
        self.tasks
            ...
            .filter(|t| {
                t.deps
                    .iter()
                    .all(|d| self.tasks.get(d).map(|dt| dt.is_success()).unwrap_or(false))
            })
            .map(|t| t.id)
            .collect()
    }
    ...
    pub fn complete_task(&mut self, id: u64, result: u64, ts: u64) {
        if let Some(t) = self.tasks.get_mut(&id) {
            ...
            self.cache.insert(nh, CacheEntry {
                input_hash: nh,
                result_hash: result,
                ts,
                hits: 0,
    ...
    pub fn fail_task(&mut self, id: u64, ts: u64) {
        ...
        if let Some(t) = self.tasks.get(&id) {
            if t.status == GraphTaskStatus::Failed {
                ...
                match policy {
                    Some(FailurePolicy::CancelDependents)
                    | Some(FailurePolicy::RetryThenCancel(_)) => {
                        for d in dependents {
                            self.cancel_subtree(d);
                        }
                    },
                    Some(FailurePolicy::SkipDependents) => {
                        for d in dependents {
                            if let Some(dt) = self.tasks.get_mut(&d) {
                                dt.skip();
                            }
                        }
                    },
        ...
        for &id in &self.topo_order {
            ...
            let max_dep = self
                ...
                .map(|t| {
                    t.deps
                        .iter()
                        .map(|d| longest.get(d).unwrap_or(0))
                        .max()
                        .unwrap_or(0)
                })
                .unwrap_or(0);
            let max_dep_id = self.tasks.get(&id).and_then(|t| {
                t.deps
                    .iter()
                    .max_by_key(|d| longest.get(d).unwrap_or(0))
                    .copied()
            });
    ...
    pub fn execution_levels(&self) -> Vec<ExecLevel> {
        ...
        for &id in &self.topo_order {
            let d = self
                ...
                .map(|t| {
                    t.deps
                        .iter()
                        .map(|dep| depth.get(dep).unwrap_or(0) + 1)
                        .max()
                        .unwrap_or(0)
                })
                .unwrap_or(0);
        ...
        levels
            .into_iter()
            .map(|(level, tasks)| ExecLevel { level, tasks })
            .collect()
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.running = self
            .tasks
            .values()
            .filter(|t| t.status == GraphTaskStatus::Running)
            .count();
        ...
        if self.stats.critical_path_ns > 0 {
            self.stats.parallelism =
                self.stats.total_cost_ns as f64 / self.stats.critical_path_ns as f64;
        }
    }

    #[inline(always)]
    pub fn task(&self, id: u64) -> Option<&GraphTask> {
        self.tasks.get(&id)
    }
    #[inline(always)]
