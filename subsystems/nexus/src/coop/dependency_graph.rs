...
impl CoopDependencyGraph {
        ...
        self.nodes
            ...
            .filter(|n| {
                !n.completed
                    && n.in_edges
                        .iter()
                        .all(|dep| self.nodes.get(dep).map(|d| d.completed).unwrap_or(true))
            })
            .map(|n| n.id)
            .collect()
    }
    ...
    pub fn stats(&self) -> DepGraphStats {
        ...
        DepGraphStats {
            ...
            max_depth,
            cycles_detected: self.cycles.len() as u64,
            ready_nodes: ready,
        }
    }
}
