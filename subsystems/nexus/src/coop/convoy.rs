...
impl ConvoyEntry {
    ...
    pub fn check_convoy(&mut self, threshold_waiters: u32, threshold_ns: u64) {
        if self.waiter_count >= threshold_waiters && self.max_wait_ns >= threshold_ns {
            ...
            self.severity = if self.waiter_count >= threshold_waiters * 4 {
                ConvoySeverity::Critical
            } else if self.waiter_count >= threshold_waiters * 3 {
                ConvoySeverity::Severe
            } else if self.waiter_count >= threshold_waiters * 2 {
                ConvoySeverity::Moderate
            } else {
                ConvoySeverity::Mild
            };
        } else {
...
impl CoopConvoy {
    pub fn new(waiter_thresh: u32, wait_thresh: u64) -> Self {
        Self {
            locks: BTreeMap::new(),
            waiter_threshold: waiter_thresh,
            wait_threshold_ns: wait_thresh,
        }
    }

    #[inline(always)]
    pub fn monitor(&mut self, lock_id: u64) {
        self.locks.insert(lock_id, ConvoyEntry::new(lock_id));
    }

    #[inline(always)]
    pub fn record_acquire(&mut self, lock_id: u64, wait_ns: u64) {
        if let Some(e) = self.locks.get_mut(&lock_id) {
            e.record_acquire(wait_ns);
        }
    }
    ...
    pub fn stats(&self) -> ConvoyStats {
        ...
        let crit = self
            .locks
            .values()
            .filter(|e| e.severity == ConvoySeverity::Critical)
            .count() as u32;
        let wait: u64 = self.locks.values().map(|e| e.total_wait_ns).sum();
        ConvoyStats {
            monitored_locks: self.locks.len() as u32,
            convoys_detected: det,
            critical_convoys: crit,
            total_wait_ns: wait,
        }
    }
}
