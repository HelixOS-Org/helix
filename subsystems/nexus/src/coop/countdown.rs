...
impl CountdownLatch {
    ...
    pub fn elapsed(&self) -> u64 {
        if self.reached_at > 0 {
            self.reached_at - self.created_at
        } else {
            0
        }
    }
...
impl CoopCountdown {
    ...
    pub fn stats(&self) -> CountdownStats {
        let active = self
            .latches
            .values()
            .filter(|l| l.state == CountdownState::Waiting)
            .count() as u32;
        let reached = self
            .latches
            .values()
            .filter(|l| l.state == CountdownState::Reached)
            .count() as u32;
        let decs: u64 = self.latches.values().map(|l| l.decrements).sum();
        let avg = if self.latches.is_empty() {
            0.0
        } else {
            self.latches.values().map(|l| l.progress()).sum::<f64>() / self.latches.len() as f64
        };
        CountdownStats {
            total_latches: self.latches.len() as u32,
            active,
            reached,
            total_decrements: decs,
            avg_progress: avg,
        }
    }
}
