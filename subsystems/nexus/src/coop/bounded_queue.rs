...
impl BoundedQueue {
    ...
    fn update_state(&mut self) {
        if self.items.is_empty() {
            self.state = BoundedQueueState::Empty;
        } else if self.items.len() >= self.capacity as usize {
            self.state = BoundedQueueState::Full;
        } else {
            self.state = BoundedQueueState::Partial;
        }
    }

    #[inline(always)]
    pub fn close(&mut self) {
        self.state = BoundedQueueState::Closed;
    }
...
impl CoopBoundedQueue {
    ...
    pub fn stats(&self) -> BoundedQueueStats {
        ...
        let avg = if self.queues.is_empty() {
            0.0
        } else {
