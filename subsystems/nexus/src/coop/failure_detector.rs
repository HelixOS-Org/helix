...
impl HeartbeatWindow {
    ...
    pub fn record(&mut self, ts: u64) {
        if self.last_heartbeat_ts > 0 {
            ...
            if self.intervals.len() > self.max_size {
    ...
    pub fn phi(&self, now: u64) -> f64 {
        if self.intervals.is_empty() || self.last_heartbeat_ts == 0 {
            return 0.0;
        }
        ...
        if p <= 0.0 {
            ...
            -libm::log(p) / core::f64::consts::LN_10
        }
    }
}
...
impl NodeDetectorState {
    pub fn new(node_id: u64, method: DetectionMethod) -> Self {
        Self {
            node_id,
            assessment: HealthAssessment::Healthy,
            heartbeat_window: HeartbeatWindow::new(100),
            phi_value: 0.0,
            phi_threshold: 8.0,
            fixed_timeout_ns: 5_000_000_000,
            ping_attempts: 0,
            pong_received: 0,
            consecutive_misses: 0,
            last_assessment_ts: 0,
    ...
    pub fn evaluate(&mut self, now: u64) -> HealthAssessment {
        ...
        match self.detection_method {
            DetectionMethod::PhiAccrual => {
                ...
                if self.phi_value >= self.phi_threshold * 2.0 {
                    self.assessment = HealthAssessment::Down;
                } else if self.phi_value >= self.phi_threshold {
                    self.assessment = HealthAssessment::ProbablyDown;
                } else if self.phi_value >= self.phi_threshold * 0.5 {
                    self.assessment = HealthAssessment::Suspect;
                } else {
                    self.assessment = HealthAssessment::Healthy;
                }
            },
            DetectionMethod::FixedTimeout => {
                ...
                if elapsed > self.fixed_timeout_ns * 3 {
                    self.assessment = HealthAssessment::Down;
                } else if elapsed > self.fixed_timeout_ns * 2 {
                    self.assessment = HealthAssessment::ProbablyDown;
                } else if elapsed > self.fixed_timeout_ns {
                    self.assessment = HealthAssessment::Suspect;
                } else {
                    self.assessment = HealthAssessment::Healthy;
                }
            ...
            DetectionMethod::Adaptive => {
                ...
                if elapsed > adaptive_timeout * 3.0 {
                    self.assessment = HealthAssessment::Down;
                } else if elapsed > adaptive_timeout * 2.0 {
                    self.assessment = HealthAssessment::ProbablyDown;
                } else if elapsed > adaptive_timeout {
                    self.assessment = HealthAssessment::Suspect;
                } else {
                    self.assessment = HealthAssessment::Healthy;
                }
...
impl PartitionDetector {
    ...
    pub fn detect(&mut self, reachability: &BTreeMap<u64, Vec<u64>>, now: u64) {
        ...
        for &start in reachability.keys() {
            if visited.get(start).unwrap_or(true) {
                continue;
            }
            let mut component = Vec::new();
            let mut stack = alloc::vec![start];
            while let Some(n) = stack.pop() {
                if visited.get(n).unwrap_or(true) {
                    continue;
                }
                ...
                if let Some(neighbors) = reachability.get(&n) {
                    for &nb in neighbors {
                        if !visited.get(nb).unwrap_or(true) {
                            stack.push(nb);
                        }
                    }
                }
        ...
        if self.is_partitioned && !was_partitioned {
            self.partition_ts = Some(now);
        }
        if !self.is_partitioned && was_partitioned {
            self.merge_ts = Some(now);
        }
    }
}
...
impl CoopFailureDetector {
    pub fn new(method: DetectionMethod) -> Self {
        Self {
            nodes: BTreeMap::new(),
            partition: PartitionDetector::new(),
            stats: FailureDetectorStats::default(),
    ...
    pub fn add_node(&mut self, node_id: u64) {
        self.nodes
            .entry(node_id)
            .or_insert_with(|| NodeDetectorState::new(node_id, self.default_method));
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.healthy_nodes = self
            .nodes
            .values()
            .filter(|n| n.assessment == HealthAssessment::Healthy)
            .count();
        self.stats.suspect_nodes = self
            .nodes
            .values()
            .filter(|n| n.assessment == HealthAssessment::Suspect)
            .count();
        self.stats.down_nodes = self
            .nodes
            .values()
            .filter(|n| {
                n.assessment == HealthAssessment::Down
                    || n.assessment == HealthAssessment::ProbablyDown
            })
            .count();
        let phis: Vec<f64> = self.nodes.values().map(|n| n.phi_value).collect();
        self.stats.avg_phi = if phis.is_empty() {
            0.0
        } else {
    ...
    #[inline(always)]
    pub fn node(&self, id: u64) -> Option<&NodeDetectorState> {
        self.nodes.get(&id)
    }
    #[inline(always)]
