...
impl WorkQueue {
    ...
    pub fn push(&mut self, task: StealTask) -> bool {
        if self.tasks.len() as u32 >= self.capacity {
            return false;
        }
...
impl CoopTaskSteal {
    ...
    pub fn add_queue(&mut self, cpu_id: u32, capacity: u32) {
        self.queues
            .entry(cpu_id)
            .or_insert_with(|| WorkQueue::new(cpu_id, capacity));
    }
    ...
    pub fn balance(&mut self) {
        let loads: Vec<(u32, u32)> = self.queues.iter().map(|(&id, q)| (id, q.load())).collect();
        if loads.len() < 2 {
            return;
        }
    ...
    pub fn stats(&self) -> TaskStealStats {
        ...
        let avg = if loads.is_empty() {
            ...
            total_tasks as f64 / loads.len() as f64
        };
