...
impl CoopTxnLog {
    ...
    fn active_segment(&mut self, ts: u64) -> &mut LogSegment {
        if self.segments.is_empty() || self.segments.last().map(|s| s.sealed).unwrap_or(true) {
            ...
            self.segments
                .push(LogSegment::new(id, self.current_lsn, ts));
        }
    ...
    #[inline(always)]
    pub fn txn(&self, id: u64) -> Option<&TxnMeta> {
        self.txns.get(&id)
    }
    #[inline(always)]
