...
impl PeerHeartbeat {
    pub fn new(id: u64, interval_ns: u64, ts: u64) -> Self {
        Self {
            ...
            last_phi: 0.0,
            send_interval_ns: interval_ns,
            heartbeats_received: 0,
            heartbeats_sent: 0,
            consecutive_misses: 0,
            last_payload_version: 0,
            registered_ts: ts,
        }
    }
...
impl CoopHeartbeatV2 {
    ...
    pub fn register_peer(&mut self, id: u64, ts: u64) {
        self.peers
            .insert(id, PeerHeartbeat::new(id, self.default_interval_ns, ts));
    }

    #[inline(always)]
    pub fn unregister_peer(&mut self, id: u64) {
    ...
    pub fn receive_heartbeat(&mut self, peer_id: u64, ts: u64, payload_version: u64) {
        if let Some(p) = self.peers.get_mut(&peer_id) {
            p.receive_heartbeat(ts, payload_version);
        }
    }
    ...
    pub fn failed_peers(&self) -> Vec<u64> {
        self.peers
            .iter()
            .filter(|(_, p)| p.status == HeartbeatStatus::Failed)
            .map(|(&id, _)| id)
            .collect()
    }
    ...
    pub fn suspected_peers(&self) -> Vec<u64> {
        self.peers
            .iter()
            .filter(|(_, p)| p.status == HeartbeatStatus::Suspected)
            .map(|(&id, _)| id)
            .collect()
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.alive_peers = self
            .peers
            .values()
            .filter(|p| p.status == HeartbeatStatus::Alive)
            .count();
        self.stats.suspected_peers = self
            .peers
            .values()
            .filter(|p| p.status == HeartbeatStatus::Suspected)
            .count();
        self.stats.failed_peers = self
            .peers
            .values()
            .filter(|p| p.status == HeartbeatStatus::Failed)
            .count();
        self.stats.unknown_peers = self
            .peers
            .values()
            .filter(|p| p.status == HeartbeatStatus::Unknown)
            .count();
        ...
        if !self.peers.is_empty() {
            self.stats.avg_phi =
                self.peers.values().map(|p| p.last_phi).sum::<f64>() / self.peers.len() as f64;
            self.stats.max_phi = self
                .peers
                .values()
                .map(|p| p.last_phi)
    ...
    #[inline(always)]
    pub fn peer(&self, id: u64) -> Option<&PeerHeartbeat> {
        self.peers.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &HeartbeatV2Stats {
        &self.stats
    }
}
