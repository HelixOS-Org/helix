...
impl Permit {
    pub fn new(id: u64, pool_id: u64, owner: u64, ptype: PermitType, now: u64) -> Self {
        let weight = match ptype {
            PermitType::Weighted(w) => w,
            PermitType::Exclusive => u32::MAX,
            _ => 1,
        };
        Self {
            id,
    ...
    pub fn revoke(&mut self) {
        self.state = PermitState::Revoked;
    }
    #[inline(always)]
    pub fn expire(&mut self) {
        self.state = PermitState::Expired;
    }

    #[inline(always)]
    pub fn hold_time(&self, now: u64) -> u64 {
...
impl PermitPool {
    ...
    pub fn try_acquire(&mut self, tid: u64, ptype: PermitType, now: u64) -> Option<u64> {
        let weight = match ptype {
            PermitType::Weighted(w) => w,
            PermitType::Exclusive => self.capacity,
            _ => 1,
        };
    ...
    pub fn release(&mut self, permit_id: u64) -> bool {
        if let Some(pos) = self
            .permits
            .iter()
            .position(|p| p.id == permit_id && p.state == PermitState::Acquired)
        {
    ...
    pub fn stats(&self) -> PermitPoolStats {
        let active: u32 = self
            ...
            .map(|p| {
                p.permits
                    .iter()
                    .filter(|pr| pr.state == PermitState::Acquired)
                    .count() as u32
            })
            .sum();
