...
impl LogEntry {
    ...
    pub fn commit(&mut self, ts: u64) {
        self.state = LogEntryState::Committed;
        self.committed_ts = Some(ts);
    }
    #[inline(always)]
    pub fn apply(&mut self, ts: u64) {
        self.state = LogEntryState::Applied;
        self.applied_ts = Some(ts);
    }
    #[inline(always)]
    pub fn compact(&mut self) {
        self.state = LogEntryState::Compacted;
    }

    #[inline(always)]
    pub fn commit_latency_ns(&self) -> Option<u64> {
        self.committed_ts
            .map(|ct| ct.saturating_sub(self.created_ts))
    }
...
impl ReplicationProgress {
    ...
    #[inline(always)]
    pub fn lag(&self, leader_last: u64) -> u64 {
        leader_last.saturating_sub(self.match_index)
    }
}
...
impl CoopConsensusLog {
    pub fn new() -> Self {
        Self {
            ...
            snapshots: Vec::new(),
            stats: ConsensusLogStats::default(),
            current_term: 0,
            commit_index: 0,
            last_applied: 0,
            first_index: 1,
    ...
    pub fn append(&mut self, term: u64, etype: LogEntryType, proposer: u64, ts: u64) -> u64 {
        ...
        self.entries
            .insert(index, LogEntry::new(index, term, etype, proposer, ts));
    ...
    pub fn apply_up_to(&mut self, index: u64, ts: u64) {
        for i in (self.last_applied + 1)..=index.min(self.commit_index) {
            if let Some(entry) = self.entries.get_mut(&i) {
                if entry.state == LogEntryState::Committed {
                    entry.apply(ts);
                }
            }
        }
    ...
    pub fn take_snapshot(&mut self, index: u64, term: u64, size: u64, ts: u64) {
        self.snapshots
            .push(SnapshotMeta::new(index, term, size, ts));
    ...
    pub fn add_follower(&mut self, node_id: u64) {
        ...
        self.followers
            .insert(node_id, ReplicationProgress::new(node_id, next));
    }
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.committed_entries = self
            .entries
            .values()
            .filter(|e| e.state == LogEntryState::Committed || e.state == LogEntryState::Applied)
            .count();
        self.stats.applied_entries = self
            .entries
            .values()
            .filter(|e| e.state == LogEntryState::Applied)
            .count();
        self.stats.pending_entries = self
            .entries
            .values()
            .filter(|e| e.state == LogEntryState::Pending)
            .count();
        ...
        self.stats.snapshot_index = self
            .snapshots
            .last()
        ...
        self.stats.max_lag = self
            .followers
            .values()
            .map(|f| f.lag(last))
            .max()
            .unwrap_or(0);
        let commit_lats: Vec<u64> = self
            .entries
            .values()
            .filter_map(|e| e.commit_latency_ns())
            .collect();
        self.stats.avg_commit_latency_ns = if commit_lats.is_empty() {
            0.0
        } else {
    ...
    #[inline(always)]
    pub fn stats(&self) -> &ConsensusLogStats {
        &self.stats
    }
}
