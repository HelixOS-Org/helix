...
impl Shard {
    ...
    #[inline(always)]
    pub fn record_read(&mut self, ts: u64) {
        self.read_ops += 1;
...
impl CoopSharding {
    pub fn new(strategy: ShardStrategy) -> Self {
        Self {
            ...
            migrations: Vec::new(),
            strategy,
            hot_config: HotShardConfig::default(),
            next_shard_id: 1,
            next_migration_id: 1,
            vnodes_per_node: 150,
    ...
    pub fn lookup(&self, key: u64) -> Option<u32> {
        match self.strategy {
            ...
            ShardStrategy::RangeBased => self
                .shards
                .values()
                .find(|s| s.contains_range(key))
                .map(|s| s.shard_id),
    ...
    pub fn detect_hot_shards(&self) -> Vec<u32> {
        ...
        let avg_ops: f64 = self
            .shards
            .values()
            .map(|s| s.total_ops() as f64)
            .sum::<f64>()
            / self.shards.len() as f64;
        self.shards
            .values()
            .filter(|s| {
                s.total_ops() as f64 > avg_ops * self.hot_config.load_multiplier
    ...
    pub fn begin_migration(&mut self, shard_id: u32, to_node: u64, ts: u64) -> Option<u64> {
        ...
        let mid = self.next_migration_id;
    ...
    pub fn complete_migration(&mut self, migration_id: u64, ts: u64) {
            ...
            .find(|m| m.migration_id == migration_id)
        {
            ...
            if let Some(n) = self.nodes.get_mut(&from) {
                n.shards_assigned.retain(|&s| s != shard_id);
            }
            if let Some(n) = self.nodes.get_mut(&to) {
                n.shards_assigned.push(shard_id);
            }
            self.rebuild_ring();
        }
    }
    ...
    pub fn recompute(&mut self) {
        ...
        if !self.shards.is_empty() {
            ...
            self.stats.load_imbalance = if self.stats.avg_load_score > 0.0 {
                (max - min) / self.stats.avg_load_score
            } else {
                0.0
            };
        }
