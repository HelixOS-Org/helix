...
impl PriorityBucket {
    pub fn new(urgency: TaskUrgency, max_size: usize) -> Self {
        Self { urgency, items: Vec::new(), total_enqueued: 0, total_dequeued: 0, max_size }
    }
    ...
    pub fn dequeue(&mut self) -> Option<u64> {
        if self.items.is_empty() { None } else { self.total_dequeued += 1; Some(self.items.remove(0)) }
    }
...
pub struct PriorityQueueV2 {
    pub id: u64,
    pub bins: BTreeMap<u8, Vec<PriorityItemV2>>,
    pub total_enqueued: u64,
    pub total_dequeued: u64,
...
impl PriorityQueueV2 {
    ...
    pub fn enqueue(&mut self, item: PriorityItemV2) -> bool {
        ...
        self.bins.entry(bin).or_insert_with(Vec::new).push(item);
        self.total_enqueued += 1;
        true
    }
