...
pub enum ConsistencyModel { Eventual, Causal, Sequential, Linearizable }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SyncPhase { Idle, Collecting, Flushing, Verifying, Complete }
...
impl SyncBarrier {
    ...
    pub fn progress(&self) -> f64 {
        if self.participants.is_empty() { return 1.0; }
...
impl MsyncCoopManager {
    ...
    pub fn create_barrier(
        &mut self, participants: Vec<u64>, consistency: ConsistencyModel, now: u64,
    ) -> u64 {
        let id = self.next_barrier;
        self.next_barrier += 1;
        self.barriers.insert(id, SyncBarrier {
            barrier_id: id, participants, arrived: Vec::new(),
            phase: SyncPhase::Collecting, created_at: now, consistency,
        });
    ...
    pub fn mark_dirty(&mut self, region_key: u64, pid: u64, pages: u64, total: u64) {
        let region = self.dirty_regions.entry(region_key).or_insert(DirtyRegion {
            owner_pid: pid, base_addr: region_key,
            dirty_pages: 0, total_pages: total,
            last_flush: 0, flush_count: 0, writeback_bytes: 0,
        });
    ...
    pub fn causally_before(&self, pid_a: u64, pid_b: u64) -> bool {
        ...
        for (k, &va) in clock_a {
            ...
            if va > vb { all_leq = false; break; }
            if va < vb { some_lt = true; }
        }
        all_leq && some_lt
    }

    pub fn barrier(&self, id: u64) -> Option<&SyncBarrier> { self.barriers.get(&id) }
    pub fn stats(&self) -> &MsyncCoopStats { &self.stats }
}
