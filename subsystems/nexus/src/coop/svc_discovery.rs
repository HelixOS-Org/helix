...
impl ServiceDescriptor {
    ...
    pub fn add_endpoint(&mut self, ep_id: u64) {
        if !self.endpoints.contains(&ep_id) {
            self.endpoints.push(ep_id);
        }
    }
...
impl CoopServiceDiscovery {
    ...
    pub fn register_service(&mut self, name: String, version: u64, ts: u64) -> u64 {
        ...
        self.services
            .entry(id)
            .or_insert_with(|| ServiceDescriptor::new(name, version, ts));
        id
    }
    ...
    pub fn add_endpoint(&mut self, service_id: u64, node: u64, address: u64, port: u16) -> u64 {
        ...
        self.next_id += 1;
        self.endpoints
            .insert(ep_id, ServiceEndpoint::new(ep_id, node, address, port));
        if let Some(svc) = self.services.get_mut(&service_id) {
            svc.add_endpoint(ep_id);
        }
        ep_id
    }
    ...
    pub fn resolve(&mut self, service_id: u64) -> Option<u64> {
        ...
        match svc.lb_strategy {
            ...
            LbStrategy::LeastConnections => available
                .iter()
                .min_by_key(|&&ep| {
                    self.endpoints
                        .get(&ep)
                        .map_or(u32::MAX, |e| e.active_connections)
                })
                .copied(),
            LbStrategy::LeastLatency => available
                .iter()
                .min_by_key(|&&ep| {
                    self.endpoints
                        .get(&ep)
                        .map_or(u64::MAX, |e| e.avg_latency_ns)
                })
                .copied(),
            LbStrategy::Random => {
    ...
    pub fn health_check(&mut self, ep_id: u64, passed: bool, ts: u64) {
        if let Some(ep) = self.endpoints.get_mut(&ep_id) {
            if passed {
                ep.health_check_pass(ts);
            } else {
                ep.health_check_fail(ts);
            }
        }
    }
    ...
    pub fn add_dependency(&mut self, from: String, to: String) {
        self.dep_edges.push(ServiceDepEdge {
            from_service: from,
            to_service: to,
            call_count: 0,
            avg_latency_ns: 0,
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.healthy_endpoints = self
            .endpoints
            .values()
            .filter(|e| e.state == ServiceState::Healthy)
            .count();
        self.stats.unhealthy_endpoints = self
            .endpoints
            .values()
            .filter(|e| e.state == ServiceState::Unhealthy)
            .count();
        ...
        let lats: Vec<u64> = self
            .endpoints
            .values()
            .filter(|e| e.avg_latency_ns > 0)
            .map(|e| e.avg_latency_ns)
            .collect();
    ...
    #[inline(always)]
    pub fn service(&self, id: u64) -> Option<&ServiceDescriptor> {
        self.services.get(&id)
    }
    #[inline(always)]
    pub fn endpoint(&self, id: u64) -> Option<&ServiceEndpoint> {
        self.endpoints.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &ServiceDiscoveryStats {
        &self.stats
    }
}
