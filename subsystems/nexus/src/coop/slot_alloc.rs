...
impl TimeSlot {
    ...
    #[inline(always)]
    pub fn usable_ns(&self) -> u64 {
        self.duration_ns.saturating_sub(self.guard_ns)
    }
    ...
    pub fn record_usage(&mut self, used: u64) {
        ...
        if used < usable {
            self.wasted_ns += usable - used;
        }
        self.utilization = if usable == 0 {
            0.0
        } else {
    ...
    pub fn end_ns(&self) -> u64 {
        self.offset_ns + self.duration_ns
    }
}
...
impl CoopSlotAllocator {
    ...
    pub fn create_frame(&mut self, period_ns: u64, num_slots: u32) -> u64 {
        ...
        for i in 0..num_slots {
            ...
            self.slots
                .insert(slot_id, TimeSlot::new(slot_id, frame_id, offset, slot_dur));
    ...
    pub fn add_node(&mut self, node_id: u64, weight: u32) {
        self.nodes
            .entry(node_id)
            .or_insert_with(|| NodeAllocation::new(node_id, weight));
    }
        ...
        if let Some(slot) = self.slots.get_mut(&slot_id) {
            ...
            if let Some(nid) = owner {
                if let Some(n) = self.nodes.get_mut(&nid) {
                    n.assigned_slots.retain(|&s| s != slot_id);
                }
            }
        }
    }
    ...
    pub fn lend_slot(&mut self, slot_id: u64, borrower: u64) -> bool {
        if let Some(slot) = self.slots.get_mut(&slot_id) {
            ...
            if let Some(n) = self.nodes.get_mut(&lender) {
                n.lent_slots.push(slot_id);
            }
            if let Some(n) = self.nodes.get_mut(&borrower) {
                n.borrowed_slots.push(slot_id);
            }
            return true;
        }
        false
    ...
    pub fn record_usage(&mut self, slot_id: u64, used_ns: u64) {
        if let Some(slot) = self.slots.get_mut(&slot_id) {
            slot.record_usage(used_ns);
        }
    }

    pub fn fair_allocate(&mut self, frame_id: u64) {
        let total_weight: u32 = self.nodes.values().map(|n| n.weight).sum();
        let frame_slots: Vec<u64> = self
            .frames
            .get(&frame_id)
            .map(|f| f.slots.clone())
            .unwrap_or_default();
        let free: Vec<u64> = frame_slots
            .iter()
            .filter(|&&s| {
                self.slots
                    .get(&s)
                    .map_or(false, |sl| sl.state == SlotState::Free)
            })
            .copied()
            .collect();
    ...
    pub fn recompute(&mut self) {
        ...
        self.stats.free_slots = self
            .slots
            .values()
            .filter(|s| s.state == SlotState::Free)
            .count();
        self.stats.assigned_slots = self
            .slots
            .values()
            .filter(|s| s.state == SlotState::Assigned || s.state == SlotState::Active)
            .count();
        self.stats.borrowed_slots = self
            .slots
            .values()
            .filter(|s| s.state == SlotState::Borrowed)
            .count();
    ...
    #[inline(always)]
    pub fn slot(&self, id: u64) -> Option<&TimeSlot> {
        self.slots.get(&id)
    }
    #[inline(always)]
    pub fn frame(&self, id: u64) -> Option<&SlotFrame> {
        self.frames.get(&id)
    }
    #[inline(always)]
    pub fn stats(&self) -> &SlotAllocStats {
        &self.stats
    }
}
