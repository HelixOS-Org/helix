...
use alloc::vec::Vec;
use super::types::{Matrix, AttentionMask, AttentionOutput};
...
impl ScaledDotProductAttention {
    ...
    pub fn forward(
        ...
        let mut scores = query.matmul(&k_t)
    ...
    pub fn forward(
        ...
        for q_start in (0..seq_len_q).step_by(self.query_chunk_size) {
            ...
            for k_start in (0..seq_len_k).step_by(self.key_chunk_size) {
                ...
                let mut scores = q_chunk.matmul(&k_t)
...
impl RelativePositionAttention {
    ...
    pub fn new(d_k: usize, max_distance: usize) -> Self {
        ...
        let position_embeddings = Matrix::random(n_positions, embed_dim, 42)
            .scale(0.1);
    ...
    pub fn forward(
        ...
        let k_t = key.transpose();
        let mut content_scores = query.matmul(&k_t)
        ...
        // Combine
        let mut scores = content_scores.add(&rel_scores)
            .unwrap_or_else(|| Matrix::new(seq_len, seq_len));
...
mod tests {
    ...
    fn test_rope() {
        ...
        assert!((rotated.get(0, 0) - x.get(0, 0)).abs() > 1e-10 
