...
use super::types::{AttentionMask, Matrix};
    ...
    pub fn forward(
        ...
        for kv_start in (0..seq_len_kv).step_by(self.block_size_kv) {
            ...
            for q_start in (0..seq_len_q).step_by(self.block_size_q) {
                ...
                // Apply mask
                self.apply_block_mask(&mut scores, mask, q_start, kv_start);
...
impl SlidingWindowFlashAttention {
    ...
    pub fn forward(&self, query: &Matrix, key: &Matrix, value: &Matrix, causal: bool) -> Matrix {
        let seq_len_q = query.rows;
        let seq_len_kv = key.rows;
        let head_dim = query.cols;
        let value_dim = value.cols;
...
impl PagedAttention {
    ...
    /// Write to cache
    pub fn write(&mut self, seq_id: usize, position: usize, key: &[f64], value: &[f64]) -> bool {
    ...
    /// Read from cache
    pub fn read(&self, seq_id: usize, position: usize) -> Option<(Vec<f64>, Vec<f64>)> {
        let logical_page = position / self.page_size;
        let offset = position % self.page_size;
...
mod tests {
    ...
    fn test_flash_attention() {
        let flash = FlashAttention::new(64).with_block_size(16, 16);
    ...
    fn test_flash_attention_causal() {
        let flash = FlashAttention::new(32).with_block_size(8, 8);
