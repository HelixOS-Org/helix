...
impl RustParser {
    ...
    pub fn tokenize(&self, source: &str) -> Vec<Token> {
        ...
        while let Some(&c) = chars.peek() {
            ...
            match c {
                ...
                '/' => {
                    ...
                    if chars.peek() == Some(&'/') {
                        ...
                        tokens.push(Token {
                            ...
                            span: Span { line, start: start_col, end: column },
                        });
                    }
                }
                ...
                // Punctuation
                '{' => { chars.next(); tokens.push(Token { kind: TokenKind::LBrace, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '}' => { chars.next(); tokens.push(Token { kind: TokenKind::RBrace, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '(' => { chars.next(); tokens.push(Token { kind: TokenKind::LParen, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                ')' => { chars.next(); tokens.push(Token { kind: TokenKind::RParen, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '[' => { chars.next(); tokens.push(Token { kind: TokenKind::LBracket, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                ']' => { chars.next(); tokens.push(Token { kind: TokenKind::RBracket, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                ';' => { chars.next(); tokens.push(Token { kind: TokenKind::Semi, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                ':' => { chars.next(); tokens.push(Token { kind: TokenKind::Colon, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                ',' => { chars.next(); tokens.push(Token { kind: TokenKind::Comma, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '.' => { chars.next(); tokens.push(Token { kind: TokenKind::Dot, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                
                // Operators
                '+' => { chars.next(); tokens.push(Token { kind: TokenKind::Plus, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '-' => { 
                    chars.next(); 
                    if chars.peek() == Some(&'>') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::Arrow, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::Minus, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '*' => { chars.next(); tokens.push(Token { kind: TokenKind::Star, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '=' => { 
                    chars.next();
                    if chars.peek() == Some(&'=') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::EqEq, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else if chars.peek() == Some(&'>') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::FatArrow, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::Eq, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '!' => { 
                    ...
                    if chars.peek() == Some(&'=') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::NotEq, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::Bang, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '<' => { 
                    ...
                    if chars.peek() == Some(&'=') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::LtEq, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::Lt, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '>' => { 
                    ...
                    if chars.peek() == Some(&'=') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::GtEq, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::Gt, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '&' => { 
                    ...
                    if chars.peek() == Some(&'&') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::AndAnd, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::And, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '|' => { 
                    ...
                    if chars.peek() == Some(&'|') {
                        chars.next();
                        tokens.push(Token { kind: TokenKind::OrOr, span: Span { line, start: start_col, end: column + 2 } });
                        column += 2;
                    } else {
                        tokens.push(Token { kind: TokenKind::Or, span: Span { line, start: start_col, end: column + 1 } });
                        column += 1;
                    }
                }
                '#' => { chars.next(); tokens.push(Token { kind: TokenKind::Hash, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '@' => { chars.next(); tokens.push(Token { kind: TokenKind::At, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
                '?' => { chars.next(); tokens.push(Token { kind: TokenKind::Question, span: Span { line, start: start_col, end: column + 1 } }); column += 1; }
    ...
    fn parse_function(&self, tokens: &[Token], index: usize) -> Option<(AstNode, usize)> {
        ...
        Some((AstNode {
            id: index as u64,
            kind: NodeKind::Function { name, params: Vec::new() },
    ...
    fn parse_struct(&self, tokens: &[Token], index: usize) -> Option<(AstNode, usize)> {
        ...
        Some((AstNode {
            id: index as u64,
            kind: NodeKind::Struct { name, fields: Vec::new() },
            span: tokens[index].span,
            children: Vec::new(),
            is_public: false,
        }, i))
    ...
    fn parse_enum(&self, tokens: &[Token], index: usize) -> Option<(AstNode, usize)> {
        ...
        Some((AstNode {
            id: index as u64,
            kind: NodeKind::Enum { name, variants: Vec::new() },
            span: tokens[index].span,
            children: Vec::new(),
            is_public: false,
        }, i))
    ...
    fn parse_impl(&self, tokens: &[Token], index: usize) -> Option<(AstNode, usize)> {
        ...
        Some((AstNode {
            id: index as u64,
            kind: NodeKind::Impl { type_name, trait_name: None },
            span: tokens[index].span,
            children: Vec::new(),
            is_public: false,
        }, i))
...
pub enum TokenKind {
    // Keywords
    Fn, Let, Mut, If, Else, Match, While, For, Loop, Break, Continue, Return,
    Struct, Enum, Impl, Trait, Pub, Use, Mod, Unsafe, Async, Await, True, False,
    ...
    Semi, Colon, Comma, Dot, Arrow, FatArrow, Hash, At, Question,
    
    // Operators
    Plus, Minus, Star, Slash, Eq, EqEq, NotEq, Bang,
    Lt, Gt, LtEq, GtEq, And, AndAnd, Or, OrOr,
}
...
#[derive(Debug, Clone)]
pub enum NodeKind {
    Function { name: String, params: Vec<(String, String)> },
    ...
    Match { expr: Box<AstNode>, arms: Vec<MatchArm> },
    While { condition: Box<AstNode>, body: Box<AstNode> },
    For { pattern: String, iter: Box<AstNode>, body: Box<AstNode> },
...
impl SemanticAnalyzer {
    ...
    pub fn analyze_purity(&self, _ast: &Ast) -> PurityAnalysis {
        PurityAnalysis { pure_functions: Vec::new(), impure_functions: Vec::new() }
    }
}
...
impl Semantics {
    pub fn new() -> Self {
        Self {
            ...
            purity: PurityAnalysis { pure_functions: Vec::new(), impure_functions: Vec::new() },
        }
    }
}
