        ...
        match query.query_type {
            ...
            QueryType::FindRelated(name, relation_type) => {
                ...
                if let Some(id) = source_id {
                    let related: Vec<Relation> = self
                        .relations
                        .iter()
                        .filter(|r| r.source == id || r.target == id)
                        .filter(|r| {
                            relation_type.is_none()
                                || Some(&r.relation_type) == relation_type.as_ref()
                        })
                        .cloned()
                        .collect();

                    let nodes: Vec<KnowledgeNode> = related
                        .iter()
                        .flat_map(|r| {
            ...
            QueryType::Infer(question) => {
                let inferred = self
                    .inference
                    .infer(&question, &self.nodes, &self.relations);
    ...
    pub fn integrate_code(&mut self, ast: &Ast, semantics: &Semantics) {
        for node in &ast.nodes {
            let kg_node = match &node.kind {
                super::understanding::NodeKind::Function { name, .. } => KnowledgeNode {
                    id: 0,
                    name: name.clone(),
                ...
                super::understanding::NodeKind::Struct { name, .. } => KnowledgeNode {
                    id: 0,
                    name: name.clone(),
                ...
                super::understanding::NodeKind::Mod { name } => KnowledgeNode {
                    id: 0,
                    name: name.clone(),
        ...
        for rel in &semantics.relationships {
            if let (Some(from_node), Some(to_node)) =
                (self.nodes.get(&rel.from), self.nodes.get(&rel.to))
            {
...
impl InferenceEngine {
    ...
    fn infer(
        ...
        if lower.contains("cause") || lower.contains("why") {
            // Find causal chains
            let causal_relations: Vec<Relation> = relations
                .iter()
                .filter(|r| r.relation_type == RelationType::CausedBy)
                .cloned()
                .collect();

            let related_nodes: Vec<KnowledgeNode> = causal_relations
                .iter()
                .flat_map(|r| {
                    nodes
                        .get(&r.source)
            ...
            let dep_relations: Vec<Relation> = relations
                .iter()
                .filter(|r| r.relation_type == RelationType::DependsOn)
                .cloned()
                .collect();

            let related_nodes: Vec<KnowledgeNode> = dep_relations
                .iter()
                .flat_map(|r| {
                    nodes
                        .get(&r.source)
...
impl KernelOntology {
    ...
    fn init_kernel_concepts(&mut self) {
        ...
        self.hierarchy
            .push(("Mutex".into(), "Synchronization".into()));
        self.hierarchy
            .push(("Spinlock".into(), "Synchronization".into()));
    }
