    ...
    pub fn new(id: DomainId, name: &str, domain_type: DomainType) -> Self {
        let (default_caps, max_caps, flags, isolation) = match domain_type {
            DomainType::Kernel => (
                ...
                CapabilitySet::all(),
                DomainFlags::ACTIVE
                    | DomainFlags::PRIVILEGED
                    | DomainFlags::CAN_CREATE_CHILDREN
                    | DomainFlags::CAN_DELEGATE,
                IsolationLevel::None,
            ),
            DomainType::System => (
...
impl IsolationLevel {
    ...
    pub fn allows_shared_memory(&self) -> bool {
        matches!(
            self,
            IsolationLevel::None | IsolationLevel::Light | IsolationLevel::Standard
        )
    }
...
impl SecurityManager {
    ...
    pub fn create_domain(
        ...
        if !parent_domain
            .flags
            .contains(DomainFlags::CAN_CREATE_CHILDREN)
        {
    ...
    pub fn register_task(&self, task_id: TaskId, domain_id: DomainId) -> DISResult<()> {
        ...
        if let Some(domain) = domains.get_mut(&domain_id) {
            let context = TaskSecurityContext::new(task_id, domain_id, domain.default_caps.clone());
...
mod tests {
    ...
    fn test_domain_creation() {
        ...
        let domain_id = manager
            .create_domain("test", DomainType::User, DomainId::SYSTEM)
            .unwrap();
