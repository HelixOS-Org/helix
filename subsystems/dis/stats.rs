...
impl TaskStats {
    ...
    fn calculate_burst_score(&self) -> u8 {
        ...
        let variance: u64 = self
            ...
            .sum::<u64>()
            / self.exec_time_history.len() as u64;
    ...
    fn is_periodic(&self) -> bool {
        ...
        let consistent = self.exec_time_history.iter().all(|t| {
            let diff = if t.raw() > avg {
                t.raw() - avg
            } else {
                avg - t.raw()
...
impl StatsCollector {
    ...
    pub fn update_system_tasks(&self, runnable: u32, blocked: u32, sleeping: u32) {
        self.system_stats
            .write()
            .update_task_counts(runnable, blocked, sleeping);
    }
    ...
    pub fn tick(&self, current_time: Nanoseconds) {
        ...
        self.last_collection
            .store(current_time.raw(), Ordering::Relaxed);
        self.collector_stats
            .collections
            .fetch_add(1, Ordering::Relaxed);
